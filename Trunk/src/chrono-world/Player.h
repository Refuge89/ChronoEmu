...
{
    uint8 rank;       // internal range [0..18] //wtf? 18! -BT
    int8 visualRank;
    float maxRP;
    float minRP;
    bool positive;
...
/* this could use some work (really ineffecient now) */
enum CharterTypes
{
...
struct FactionReputation
{
	...
	uint8 flag;
	int32 baseStanding;
	CHRONO_INLINE int32 CalcStanding() { return standing - baseStanding; }
	CHRONO_INLINE bool Positive() { return standing >= 0; }
};
struct PlayerInfo
{
	~PlayerInfo();
...
#define PLAYER_RATING_MODIFIER_MELEE_HASTE						PLAYER_FIELD_COMBAT_RATING_14
#define PLAYER_RATING_MODIFIER_RANGED_HASTE						PLAYER_FIELD_COMBAT_RATING_15
#define PLAYER_RATING_MODIFIER_SPELL_HASTE						PLAYER_FIELD_COMBAT_RATING_16
#define PLAYER_RATING_MODIFIER_MELEE_MAIN_HAND_SKILL			PLAYER_FIELD_COMBAT_RATING_17
#define PLAYER_RATING_MODIFIER_MELEE_OFF_HAND_SKILL				PLAYER_FIELD_COMBAT_RATING_18
// #define UNKNOWN												PLAYER_FIELD_COMBAT_RATING_1
#define PLAYER_RATING_MODIFIER_EXPERTISE						PLAYER_FIELD_COMBAT_RATING_1+24
...
public:
	...
	CHRONO_INLINE const char* GetName() { return m_name.c_str(); }
	CHRONO_INLINE std::string* GetNameString() { return &m_name; }
	void Die();
    ...
    /* Taxi                                                                 */
    /************************************************************************/
    CHRONO_INLINE TaxiPath*    GetTaxiPath() { return m_CurrentTaxiPath; }
    CHRONO_INLINE bool         GetTaxiState() { return m_onTaxi; }
    ...
    CHRONO_INLINE void         SetTaxiState    (bool state) { m_onTaxi = state; }
    CHRONO_INLINE void         SetTaximask     (uint8 index, uint32 value ) { m_taximask[index] = value; }
    CHRONO_INLINE void         SetTaxiPath     (TaxiPath *path) { m_CurrentTaxiPath = path; }
    CHRONO_INLINE void         SetTaxiPos()	{m_taxi_pos_x = m_position.x; m_taxi_pos_y = m_position.y; m_taxi_pos_z = m_position.z;}
    CHRONO_INLINE void         UnSetTaxiPos()	{m_taxi_pos_x = 0; m_taxi_pos_y = 0; m_taxi_pos_z = 0; }
	...
	CHRONO_INLINE QuestLogEntry*GetQuestLogInSlot(uint32 slot)  { return m_questlog[slot]; }
    CHRONO_INLINE uint32        GetQuestSharer()                { return m_questSharer; }
    
    CHRONO_INLINE void         SetQuestSharer(uint32 guid)     { m_questSharer = guid; }
    void                SetQuestLogSlot(QuestLogEntry *entry, uint32 slot);
    
    CHRONO_INLINE void         PushToRemovedQuests(uint32 questid)	{ m_removequests.insert(questid);}
	...
	uint32 m_questbarrier1[20];
    QuestLogEntry*      m_questlog[20];
	uint32 m_questbarrier2[20];
	...
	CHRONO_INLINE uint32 GetTeam() { return m_team; }
	CHRONO_INLINE void SetTeam(uint32 t) { m_team = t; m_bgTeam=t; }
	CHRONO_INLINE void ResetTeam() { m_team = myRace->team_id==7 ? 0 : 1; m_bgTeam=m_team; }

	CHRONO_INLINE bool IsInFeralForm()
	{
		int s = GetShapeShift();
		if( s <= 0 )
			return false;
    ...
    /* PVP                                                                  */
    /************************************************************************/
	CHRONO_INLINE uint8 GetPVPRank()
	{
		return (uint8)((GetUInt32Value(PLAYER_BYTES_3) >> 24) & 0xFF);
	}
	CHRONO_INLINE void SetPVPRank(int newrank)
	{
    ...
    CHRONO_INLINE void         SetBanned() { m_banned = 4;}
	CHRONO_INLINE void         SetBanned(string Reason) { m_banned = 4; m_banreason = Reason;}
	CHRONO_INLINE void         SetBanned(uint32 timestamp, string& Reason) { m_banned = timestamp; m_banreason = Reason; }
	CHRONO_INLINE void         UnSetBanned() { m_banned = 0; }
	CHRONO_INLINE string       GetBanReason() {return m_banreason;}
	...
	CHRONO_INLINE uint8        GetDuelStatus() { return m_duelStatus; }
	void                SetDuelState(uint8 state) { m_duelState = state; }
	CHRONO_INLINE uint8        GetDuelState() { return m_duelState; }
    // duel variables
    Player*             DuelingWith;
    ...
    /* Pets                                                                 */
    /************************************************************************/
	CHRONO_INLINE void			SetSummon(Pet *pet) { m_Summon = pet; }
	CHRONO_INLINE Pet*			GetSummon(void) { return m_Summon; }
	uint32						GeneratePetNumber(void);
	void						RemovePlayerPet(uint32 pet_number);
	CHRONO_INLINE void			AddPlayerPet(PlayerPet* pet, uint32 index) { m_Pets[index] = pet; }
	CHRONO_INLINE PlayerPet*	GetPlayerPet(uint32 idx)
	{
	...
	CHRONO_INLINE PlayerPet*	GetFirstPet(void) { return GetPlayerPet(GetFirstPetNumber()); }
	CHRONO_INLINE void			SetStableSlotCount(uint8 count) { m_StableSlotCount = count; }
	CHRONO_INLINE uint8			GetStableSlotCount(void) { return m_StableSlotCount; }
    ...
    /* Item Interface                                                       */
    /************************************************************************/
	CHRONO_INLINE ItemInterface* GetItemInterface() { return m_ItemInterface; } // Player Inventory Item storage
	CHRONO_INLINE void         ApplyItemMods(Item *item, int8 slot, bool apply,bool justdrokedown=false) {  _ApplyItemMods(item, slot, apply,justdrokedown); }
    ...
    /* Loot                                                                 */
    /************************************************************************/
	CHRONO_INLINE const uint64& GetLootGUID() const { return m_lootGuid; }
	CHRONO_INLINE void         SetLootGUID(const uint64 &guid) { m_lootGuid = guid; }
    ...
    /* World Session                                                        */
    /************************************************************************/
	CHRONO_INLINE WorldSession* GetSession() const { return m_session; }
	...
	void _LoadInventoryLight(uint32 guid);
	...
	//CHRONO_INLINE std::list<struct skilllines>getSkillLines() { return m_skilllines; }
	...
	CHRONO_INLINE float GetBlockFromSpell() { return m_blockfromspell; }
	CHRONO_INLINE float GetSpellCritFromSpell() { return m_spellcritfromspell; }
	CHRONO_INLINE float GetHitFromMeleeSpell() { return m_hitfrommeleespell; }
	CHRONO_INLINE float GetHitFromSpell() { return m_hitfromspell; }
	CHRONO_INLINE float GetParryFromSpell() { return m_parryfromspell; }
	CHRONO_INLINE float GetDodgeFromSpell() { return m_dodgefromspell; }
	...
	CHRONO_INLINE uint32 GetHealthFromSpell() { return m_healthfromspell; }
	CHRONO_INLINE uint32 GetManaFromSpell() { return m_manafromspell; }
	...
	CHRONO_INLINE void SetTalentResetTimes(uint32 value) { m_talentresettimes = value; }
	void SetPlayerStatus(uint8 pStatus) { m_status = pStatus; }
	CHRONO_INLINE uint8 GetPlayerStatus() { return m_status; }
	...
	CHRONO_INLINE uint8 GetShapeShift()
	{
	...
	CHRONO_INLINE bool IsVisible(Object* pObj) { return !(m_visibleObjects.find(pObj) == m_visibleObjects.end()); }
	void AddInRangeObject(Object* pObj);
	void OnRemoveInRangeObject(Object* pObj);
	void ClearInRangeSet();
	CHRONO_INLINE void AddVisibleObject(Object* pObj) { m_visibleObjects.insert(pObj); }
	CHRONO_INLINE void RemoveVisibleObject(Object* pObj) { m_visibleObjects.erase(pObj); }
	CHRONO_INLINE void RemoveVisibleObject(InRangeSet::iterator itr) { m_visibleObjects.erase(itr); }
	CHRONO_INLINE InRangeSet::iterator FindVisible(Object * obj) { return m_visibleObjects.find(obj); }
	CHRONO_INLINE void RemoveIfVisible(Object * obj)
	{
	...
	CHRONO_INLINE bool GetVisibility(Object * obj, InRangeSet::iterator *itr)
	{
		*itr = m_visibleObjects.find(obj);
		return ((*itr) != m_visibleObjects.end());
	}

	CHRONO_INLINE InRangeSet::iterator GetVisibleSetBegin() { return m_visibleObjects.begin(); }
	CHRONO_INLINE InRangeSet::iterator GetVisibleSetEnd() { return m_visibleObjects.end(); }
	...
	CHRONO_INLINE float res_M_crit_get(){return m_resist_critical[0];}
	CHRONO_INLINE void res_M_crit_set(float newvalue){m_resist_critical[0]=newvalue;}
	CHRONO_INLINE float res_R_crit_get(){return m_resist_critical[1];}
	CHRONO_INLINE void res_R_crit_set(float newvalue){m_resist_critical[1]=newvalue;}
...
*/
	CHRONO_INLINE uint32* GetPlayedtime() { return m_playedtime; };
    ...
    CHRONO_INLINE uint32 GetSoulStoneReceiver(){return SoulStoneReceiver;}
    CHRONO_INLINE void SetSoulStoneReceiver(uint32 StoneGUID){SoulStoneReceiver = StoneGUID;}
    CHRONO_INLINE uint32 GetSoulStone(){return SoulStone;}
    CHRONO_INLINE void SetSoulStone(uint32 StoneID){SoulStone = StoneID;}
	...
	CHRONO_INLINE bool IsMounted() {return (m_MountSpellId!=0 ? true : false); }
	...
	CHRONO_INLINE uint32 GetAreaID() { return m_AreaID; }
	...
	//hmm, didn't knew it was going to much such a mess -BT
	...
	CHRONO_INLINE void setMyCorpse(Corpse * corpse) { myCorpse = corpse; }
	CHRONO_INLINE Corpse * getMyCorpse() { return myCorpse; }
	...
	******************/
	...
	//! Is PVP flagged?
	CHRONO_INLINE bool IsPvPFlagged() { return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP); }
	CHRONO_INLINE void SetPvPFlag()
	{
	...
	//! Removal
	CHRONO_INLINE void RemovePvPFlag()
	{
		StopPvPTimer();
		RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP);
	}
	//! Do this on /pvp off
	CHRONO_INLINE void ResetPvPTimer();
	//! Stop the timer for pvp off
	CHRONO_INLINE void StopPvPTimer() { m_pvpTimer = 0; }
	...
	CHRONO_INLINE uint32 LastHonorResetTime() const { return m_lastHonorResetTime; }
	CHRONO_INLINE void LastHonorResetTime(uint32 val) { m_lastHonorResetTime = val; }
	uint32 OnlineTime;
	bool tutorialsDirty;
	LevelInfo * lvlinfo;
	...
	CHRONO_INLINE void SetName(string& name) { m_name = name; }
	...
	CHRONO_INLINE void AddComboPoints(uint64 target, uint8 count)
	{
	...
	CHRONO_INLINE void NullComboPoints() { if(!m_retainComboPoints) { m_comboTarget = 0; m_comboPoints = 0; m_spellcomboPoints=0; } UpdateComboPoints(); }
	...
	CHRONO_INLINE Player * GetTradeTarget()
	{
	...
	/* Last Speeds */
	CHRONO_INLINE void UpdateLastSpeeds()
	{
	...
	CHRONO_INLINE bool IsAttacking() {return m_attacking; }
...
public:
	...
	CHRONO_INLINE PlayerSkill* Grab() { return &m_itr->second; }
	CHRONO_INLINE bool End() { return (m_itr==m_endItr)?true:false; }
};
