...
void Spell::SpellEffectInstantKill(uint32 i)
{
	...
	*/
	...
	default:
		{
			// moar cheaters
			if( p_caster == nullptr || (u_caster != nullptr && u_caster->IsPet() ) )
				return;
...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
	...
	if(unitTarget->IsImmuneForCustomEffect(m_spellInfo->School))
	{
	...
	else 
	{
		...
		// Force dispel on Shadow Trance after cast
		case SPELL_HASH_SHADOW_BOLT: 
			{
		...
		case 8063:
            {
                ...
                uint32 deviate_id = 0;
				uint32 deviate_rand = RandomUInt(5);
                switch(deviate_rand)
                {
                    case 1: deviate_id = 8064; break;     // Sleep
                    case 2: deviate_id = 8065; break;     // Invig
                    case 3: deviate_id = 8066; break;     // Shrink
                    case 4: deviate_id = 8067; break;     // PARTY TIME, BREAK IT DOWN
                    case 5: deviate_id = 8068; break;     // Spirit buff
                }
				p_caster->CastSpell(p_caster, deviate_id, true);
					return;
            }
		...
		case 16589:
            {
                ...
                uint32 noggen_id = 0;
				uint32 noggen_rand = RandomUInt(3);
                switch(noggen_rand)
                {
                    case 1: noggen_id = 16595; break;     // Shrink
                    case 2: noggen_id = 16593; break;     // Sleep
                    case 3: noggen_id = 16591; break;     // Skeletor
                }
				p_caster->CastSpell(p_caster, noggen_id, true);
					return;
            }			
	...
	case 12472:
		{
			...
			p_caster->ClearCooldownsOnLine(6, m_spellInfo->Id);
		}break;	
		
	/*
		TALENT: Frost Warding
	*/
	case 11189:
	case 28332:
		{
...
//			u_caster->SpellNonMeleeDamageLog(unitTarget,spellId,damage,false);
	...
	*/
	...
	case 16190://Mana Tide
		{
			if(unitTarget == nullptr || unitTarget->isDead() || unitTarget->getClass() == WARRIOR || unitTarget->getClass() == ROGUE)
 				return;
	...
	case 20968: //Judgement of Command
		{
			...
			Spell * spell=new Spell(m_caster,dbcSpell.LookupEntry(SpellID),true,nullptr);
	...
	case 23725:// Gift of Life
		{
			...
			Spell * spe = new Spell(u_caster,inf,true,nullptr);
			spe->prepare(&tgt);
...
void Spell::SpellEffectApplyAura(uint32 i)  // Apply Aura
{
	...
	sLog.outDebug("Spell::SpellEffectApplyAura");
...
{
	uint32 bonusheal = u_caster->HealDoneMod[m_spellInfo->School];
	...
	if(sd->CastTime < 3500)
	{
		...
		bonusheal = float2int32( bonusheal * pct );
	}

	uint32 bonuspct = 1 + u_caster->HealDonePctMod[m_spellInfo->School] / 100.0f;
	...
	else
	{
		...
		case 18562: //druid - swiftmend
			{
				if( unitTarget )
				{
					...
					if( taura != nullptr && taura->GetSpellProto() != nullptr)
					{
						...
						Spell *spell = new Spell( m_caster, taura->GetSpellProto(), false, nullptr );				
		...
		default:
			Heal((int32)damage * bonuspct + bonusheal );
			break;
		}
	}
}
...
void Spell::SpellEffectAddExtraAttacks(uint32 i) // Add Extra Attacks
{
	...
	sLog.outDebug("Hash: %X", m_spellInfo->NameHash);
	if(m_spellInfo->NameHash == SPELL_HASH_WINDFURY_WEAPON)
	{
		sLog.outDebug("Caught Windfury!");

		for(int i = 0; i < 2; i++)
			u_caster->Strike(unitTarget, GetType(), m_spellInfo, 0, 0, 0, false, true);
		//u_caster->Strike(Victim, Type, SpellEntry, Add Dmg, Pct Dmg Mod, Exclusive Dmg, , );
...
void Spell::SpellEffectCreateItem(uint32 i) // Create item 
{
	...
	for(int j=0; j<3; j++) // now create the Items
	{
		...
		if (!add)
		{
			...
			//removed after item modifaction
...
void Spell::SpellEffectSummon(uint32 i) // Summon
{
	...
	if(m_spellInfo->EffectMiscValue[i] == 510)	// Water Elemental
	{
		...
		summon->CreateAsSummon(m_spellInfo->EffectMiscValue[i], ci, nullptr, p_caster, m_spellInfo, 1, 45000);
...
void Spell::SpellEffectEnergize(uint32 i) // Energize
{
	...
	if(m_spellInfo->Id==20272 && ProcedOnSpell)
	{
...
void Spell::SpellEffectTriggerMissile(uint32 i) // Trigger Missile
{
	...
	if (spellRadius == 0.0f)
	{
		Spell* newspell=new Spell(m_caster, spInfo, true, nullptr);
		SpellCastTargets t;

		t=m_targets;

		//use unit target
		if (m_targets.m_unitTarget && m_caster->GetMapMgr()->GetUnit(m_targets.m_unitTarget) != nullptr)
		{
	...
	for(std::set<Object*>::iterator itr = m_caster->GetInRangeSetBegin(); itr != m_caster->GetInRangeSetEnd(); itr++ )
	{
		...
		Spell*sp=new Spell(m_caster,spInfo,true,nullptr);
		SpellCastTargets tgt;
...
void Spell::SpellEffectOpenLock(uint32 i) // Open Lock
{
	...
	switch(locktype)
	{
		...
		{
			sLog.outError("herbalism");
			if(!gameObjTarget) return;
			sLog.outError("we have object entry = %i",gameObjTarget->GetEntry()); 
			...
			if(Rand(100.0f)) // 3% chance to fail//why?
			{
				...
				else
				{
					if( gameObjTarget->loot.items.size() == 0 )
					{
						sLog.outError("we don't have loot!");
						lootmgr.FillGOLoot(&gameObjTarget->loot,gameObjTarget->GetEntry(), gameObjTarget->GetMapMgr() ? (gameObjTarget->GetMapMgr()->iInstanceMode ? true : false) : false);
					}
					else
						bAlreadyUsed = true;
		...
		case LOCKTYPE_SLOW_OPEN: // used for BG go's
		{
			...
			Spell *sp=new Spell(p_caster,en,true,nullptr);
			SpellCastTargets tgt;
	...
	if( gameObjTarget != nullptr && gameObjTarget->GetUInt32Value( GAMEOBJECT_TYPE_ID ) == GAMEOBJECT_TYPE_CHEST)
...
void Spell::SpellEffectLearnSpell(uint32 i) // Learn Spell
{
	...
	if(playerTarget)
    {
        ...
        SpellEntry *spellinfo = dbcSpell.LookupEntryForced(spellToLearn);
        for(uint32 i=0;i<3;i++)
            ...
            {
                Spell *sp = new Spell(unitTarget, spellinfo, true, nullptr);
                SpellCastTargets targets;
                targets.m_unitTarget = unitTarget->GetGUID();
...
void Spell::SpellEffectDispel(uint32 i) // Dispel
{
	...
	if(unitTarget->m_auras[x])
	{
		...
		if(!aur->IsPassive() && !(aur->GetSpellProto()->Attributes & ATTRIBUTES_IGNORE_INVULNERABILITY))
		{
			...
			{
				unitTarget->HandleProc( CPROC_PRE_DISPEL , u_caster , m_spellInfo, aur->GetSpellId() );
	...
	{
		if (itr->first->m_spellScript != nullptr)
		...
		if (itr->second)
			unitTarget->RemoveAllAuras(itr->first->GetSpellProto()->Id, itr->first->GetCasterGUID());
		else
			itr->first->Remove();
...
void Spell::SpellEffectSummonGuardian(uint32 i) // Summon Guardian
{
	...
	/*if ( u_caster->GetTypeId()==TYPEID_PLAYER && itemTarget )
	{
		...
		{
			uint32 skill202 = TO_PLAYER( m_caster )->_GetSkillLineCurrent(SKILL_ENGINERING);
			if (skill202>0)
			{
				level = skill202/5;
			}
...
void Spell::SpellEffectTameCreature(uint32 i)
{
	...
	pPet->CreateAsSummon(tame->GetEntry(), tame->GetCreatureName(), tame, static_cast<Unit*>(p_caster), nullptr, 2, 0);
	//tame->SafeDelete();
	//delete tame;
...
void Spell::SpellEffectSummonPet(uint32 i) //summon - pet
{
	...
	if(ci)
	{
		...
		Pet *summon = objmgr.CreatePet();
		summon->SetInstanceID(m_caster->GetInstanceID());
		summon->CreateAsSummon(m_spellInfo->EffectMiscValue[i], ci, nullptr, u_caster, m_spellInfo, 1, 0);
	}
}

void Spell::SpellEffectWeapondamage( uint32 i ) // Weapon damage +
{
	if( unitTarget == nullptr || u_caster == nullptr )
		return;
...
void Spell::SpellEffectTriggerSpell(uint32 i) // Trigger Spell
{
	...
	Spell* sp=new Spell(m_caster,dbcSpell.LookupEntry(m_spellInfo->EffectTriggerSpell[i]),true,nullptr);
	sp->TriggerCallback = this;	
...
void Spell::SpellEffectHealMaxHealth(uint32 i)   // Heal Max Health
{
	...
	if( unitTarget->GetTypeId() == TYPEID_PLAYER)
	{
		 SendHealSpellOnPlayer( TO_PLAYER( m_caster ), TO_PLAYER( unitTarget ), dif, false );
	}
...
void Spell::SpellEffectAddFarsight(uint32 i) // Add Farsight
{
	...
	if( dynObj == nullptr ) //i <3 burlex :P
	{
		delete dynObj;
		return;
	}
...
void Spell::SpellEffectSummonPossessed(uint32 i) // eye of kilrog
{
	...
	CreatureInfo *ci = CreatureNameStorage.LookupEntry(m_spellInfo->EffectMiscValue[i] == 4277);
	if( ci)
	{
		Creature* NewSummon = m_caster->GetMapMgr()->CreateCreature(ci->Id);
		// Create
		NewSummon->SetInstanceID(m_caster->GetInstanceID());
		NewSummon->Create( "Eye of Kilrogg", m_caster->GetMapId(), 
...
void Spell::SpellEffectScriptEffect(uint32 i) // Script Effect
{
	...
	*/
	...
	case 23784:
		{
			if( p_caster == nullptr || unitTarget == nullptr)
			...
			{
				//for self
				Spell *sp = new Spell( p_caster, dbcSpell.LookupEntry( casted_spell_id ), true, nullptr );
				SpellCastTargets tgt( p_caster->GetGUID() );
				sp->prepare( &tgt );
				//for pet
				sp = new Spell(  unitTarget, dbcSpell.LookupEntry( casted_spell_id ), true, nullptr );
			...
			{
				//for self
				Spell *sp = new Spell( p_caster, dbcSpell.LookupEntry( inc_resist_by_level_spell ), true, nullptr );
				sp->forced_basepoints[0] = p_caster->GetUInt32Value( UNIT_FIELD_LEVEL ) * inc_resist_by_level / 100;
				SpellCastTargets tgt( p_caster->GetGUID() );
				sp->prepare( &tgt );
				//for pet
				sp = new Spell( unitTarget, dbcSpell.LookupEntry( inc_resist_by_level_spell ), true, nullptr );
...
void Spell::SpellEffectDuel(uint32 i) // Duel
{
	...
	*/
	...
	if (playerTarget->DuelingWith != nullptr)
	{
...
void Spell::SpellEffectSummonTotem(uint32 i) // Summon Totem
{
	...
	else
	{
		...
		pTotem->GetAIInterface()->m_totemspelltime = time;
	}
...
void Spell::SpellEffectCharge(uint32 i)
{
	...
	if(!unitTarget->isAlive())
		return;
    if (u_caster->IsStunned() || u_caster->m_auracount[SPELL_AURA_MOD_ROOT] || u_caster->IsPacified() || u_caster->IsFeared())
        return;
...
{
	if( unitTarget == nullptr || !unitTarget->isAlive() || !unitTarget->IsPlayer() )
		return;
...
{
	if(playerTarget == nullptr || !playerTarget->isAlive() || !m_caster)
		return;
...
void Spell::SpellEffectFeedPet(uint32 i)  // Feed Pet
{
	...
	Spell *sp= new Spell((Object *)p_caster,spellInfo,true,nullptr);
...
void Spell::SpellEffectSummonDemon(uint32 i)
{
	...
	if(ci)
	{
		...
		pPet->SetInstanceID(p_caster->GetInstanceID());
		pPet->CreateAsSummon(m_spellInfo->EffectMiscValue[i], ci, nullptr, p_caster, m_spellInfo, 1, 300000, this);
 	}
	...
	if(m_spellInfo->Id == 1122)
	{
		...
		Spell *sp=new Spell((Object *)pPet,spellInfo,true,nullptr);
		SpellCastTargets tgt;
		tgt.m_unitTarget=pPet->GetGUID();
		sp->prepare(&tgt);
	}
...
void Spell::SpellEffectDummyMelee( uint32 i ) // Normalized Weapon damage +
{
	...
	if( m_spellInfo->NameHash == SPELL_HASH_OVERPOWER && p_caster != nullptr ) //warrior : overpower - let us clear the event and the combopoint count
	{
	...
	else if( m_spellInfo->NameHash == SPELL_HASH_DEVASTATE)
	{
		...
		Spell *spell = new Spell(u_caster, spellInfo ,true, nullptr);
	...
	else if( m_spellInfo->NameHash == SPELL_HASH_CRUSADER_STRIKE ) // Crusader Strike - refreshes *all* judgements, not just your own
	{
		for( int x = MAX_POSITIVE_AURAS ; x <= MAX_AURAS ; x ++ ) // there are only debuff judgements anyway :P
		{
			if( unitTarget->m_auras[x] && unitTarget->m_auras[x]->GetSpellProto()->buffIndexType == SPELL_TYPE_INDEX_JUDGEMENT )
			{
				...
				aur->Remove();
				Spell * sp = new Spell( this->u_caster , spinfo , true , nullptr );
	...
	//Hemorrhage
	if( p_caster != nullptr && m_spellInfo->NameHash == SPELL_HASH_HEMORRHAGE )
...
void Spell::SpellEffectAddHonor(uint32 i)
{
    ...
    sLog.outDebug("SpellEffect::AddHonor (spell_id %u) rewards %u honor points (non scale) for player: %u", m_spellInfo->Id, damage, ((Player*)unitTarget)->GetLowGUID());
}
...
void Spell::SpellEffectTriggerSpellWithValue(uint32 i)
{
	...
	Spell*sp=new Spell(m_caster,dbcSpell.LookupEntry(TriggeredSpell->Id),true,nullptr);
