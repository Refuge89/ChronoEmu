...
public:
	...
	CHRONO_INLINE void SendPacket(StackPacket * packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}
	...
	CHRONO_INLINE uint32 GetAccountId() const { return _accountId; }
	CHRONO_INLINE Player* GetPlayer() { return _player; }
	...
	CHRONO_INLINE char* GetPermissions() { return permissions; }
	CHRONO_INLINE int GetPermissionCount() { return permissioncount; }
	CHRONO_INLINE bool HasPermissions() { return (permissioncount > 0) ? true : false; }
	CHRONO_INLINE bool HasGMPermissions()
	{
		if(!permissioncount)
			return false;
	...
	CHRONO_INLINE void SetSocket(WorldSocket *sock)
	{
		_socket = sock;
	}
	CHRONO_INLINE void SetPlayer(Player *plr) { _player = plr; }
	
	CHRONO_INLINE void SetAccountData(uint32 index, char* data, bool initial,uint32 sz)
	{
		ASSERT(index < 8);
	...
	CHRONO_INLINE AccountDataEntry* GetAccountData(uint32 index)
	{
		ASSERT(index < 8);
	...
	CHRONO_INLINE void QueuePacket(WorldPacket* packet)
	{
	...
	CHRONO_INLINE WorldSocket* GetSocket() { return _socket; }
	...
	CHRONO_INLINE void SetInstance(uint32 Instance) { instanceId = Instance; }
	CHRONO_INLINE uint32 GetLatency() { return _latency; }
	CHRONO_INLINE string GetAccountName() { return _accountName; }
	CHRONO_INLINE const char * GetAccountNameS() { return _accountName.c_str(); }

	CHRONO_INLINE uint32 GetClientBuild() { return client_build; }
	CHRONO_INLINE void SetClientBuild(uint32 build) { client_build = build; }
	bool bDeleted;
	CHRONO_INLINE uint32 GetInstance() { return instanceId; }
	Mutex deleteMutex;
	...
	CHRONO_INLINE bool IsLoggingOut() { return _loggingOut; }
...
private:
	...
	uint8 _updatecount;
public:
	CHRONO_INLINE MovementInfo* GetMovementInfo() { return &movement_info; }
