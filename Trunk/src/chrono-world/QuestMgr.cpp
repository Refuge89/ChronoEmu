...
void QuestMgr::BuildQuestDetails(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 menutype, uint32 language, Player *plr)
{
	...
	*data << qst->count_required_mob;
	for(uint32 i=0; i <  4; i++)
	{
		*data << uint32(qst->required_mob);
		*data << uint32(qst->required_mobcount);
	}
}
...
void QuestMgr::BuildQuestComplete(Player*plr, Quest* qst)
{
	...
	data << uint32(0);	// Could be Honor, Or may not even be needed O_O
	...
	for(int z = 0; z < 2; z++)
	{
		...
		if(!qst->reward_repfaction[z])
		{
			...
			if( qst_giver->GetTypeId() == TYPEID_UNIT )
				if(((Creature*)qst_giver)->m_factionDBC != nullptr )
...
//Re-Check for Gold Requirement (needed for possible xploit)
    ...
    if(IsQuestRepeatable(qst))
    {
	    ...
	    if(qst->effect_on_player)
	    {
		    ...
		    if(inf)
		    {
			    Spell * spe = new Spell(qst_giver,inf,true,nullptr);
...
uint32 QuestMgr::GenerateRewardMoney( Player *pl, Quest * qst )
{
	...
	if ( pl && pl->getLevel() == 60 && qst->is_repeatable == 0 )
		return qst->reward_money + float2int32( GenerateQuestXP( pl, qst ) ) * 6;
	else
...
bool QuestMgr::CanStoreReward(Player *plyr, Quest *qst, uint32 reward_slot)
{
    ...
    available_slots = plyr->GetItemInterface()->CalculateFreeSlots(nullptr);
    // Static Item reward
    for(uint32 i = 0; i < 4; ++i)
    {
        if(qst->reward_item[i])
