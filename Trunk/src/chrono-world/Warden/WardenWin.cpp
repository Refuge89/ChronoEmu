...
void WardenWin::Init(WorldSession *pClient, BigNumber *K)
{
    ...
    */
    ...
    sLog.outString("Server side warden for client %u initializing...", pClient->GetAccountId());
    PrintHexArray("  C->S Key: ", InputKey, 16, true);
    PrintHexArray("  S->C Key: ", OutputKey, 16, true);
    PrintHexArray("  Seed: ", Seed, 16, true);
    sLog.outString("Loading Module...");
...
{
    sLog.outString("Initialize module");
...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    sLog.outString("Request hash reply: succeed");
...
void WardenWin::RequestData()
{
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
    {
        ...
        switch (type)
        {
            ...
            case MODULE_CHECK:
            {
                uint32 seed = static_cast<uint32>(rand());
                buff << uint32(seed);
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
        stream << *itr << " ";
    sLog.outString(stream.str().c_str());
}

void WardenWin::HandleData(ByteBuffer &buff)
{
    sLog.outString("Handle data");
    ...
    //TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            sLog.outString("TIMING CHECK FAIL result 0x00");
            found = true;
        }
        ...
        sLog.outString("ServerTicks %u", ticksNow);         // now
        sLog.outString("RequestTicks %u", ServerTicks);     // at request
        sLog.outString("Ticks %u", newClientTicks);         // at response
        sLog.outString("Ticks diff %u", ourTicks - newClientTicks);
    }
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    sLog.outString("RESULT MEM_CHECK not 0x00, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    found = true;
                    continue;
                }
                ...
                {
                    sLog.outString("RESULT MEM_CHECK fail CheckId %u account Id %u", *itr, Client->GetAccountId());
                    found = true;
                ...
                buff.rpos(buff.rpos() + rd->Length);
                sLog.outString("RESULT MEM_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) != 0)
                {
                    if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                        sLog.outString("RESULT PAGE_CHECK fail, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    if (type == MODULE_CHECK)
                        sLog.outString("RESULT MODULE_CHECK fail, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    if (type == DRIVER_CHECK)
                        sLog.outString("RESULT DRIVER_CHECK fail, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    found = true;
                ...
                buff.rpos(buff.rpos() + 1);
                if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                    sLog.outString("RESULT PAGE_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                else if (type == MODULE_CHECK)
                    sLog.outString("RESULT MODULE_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                else if (type == DRIVER_CHECK)
                    sLog.outString("RESULT DRIVER_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
            case LUA_STR_CHECK:
            {
                ...
                if (Lua_Result != 0)
                {
                    sLog.outString("RESULT LUA_STR_CHECK fail, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    found = true;
                    continue;
                }
                ...
                if (luaStrLen != 0)
                {
                    ...
                    sLog.outString("Lua string: %s", str);
                    delete[] str;
                }
                buff.rpos(buff.rpos() + luaStrLen);         // skip string
                sLog.outString("RESULT LUA_STR_CHECK passed, CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    sLog.outString("RESULT MPQ_CHECK not 0x00 account id %u", Client->GetAccountId());
                    found = true;
                    continue;
                }
                ...
                {
                    sLog.outString("RESULT MPQ_CHECK fail, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    found = true;
                ...
                buff.rpos(buff.rpos() + 20);                // 20 bytes SHA1
                sLog.outString("RESULT MPQ_CHECK passed, CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
