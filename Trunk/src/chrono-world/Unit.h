...
{	//                                            Hex    Bit     Decimal  Comments
	...
	UNIT_FLAG_LOCK_PLAYER                = 0x00000004, // 3            4  Disables Movement
	UNIT_FLAG_PLAYER_CONTROLLED          = 0x00000008, // 4            8  PvP State. Attackable or not in PvP
	UNIT_FLAG_RENAME_CREATURE            = 0x00000010, // 5           16  For renaming creatures, maybe pets?
	UNIT_FLAG_UNKNOWN_6                  = 0x00000020, // 6           32  Some kind of Preperation Flag. Don't use reagents / Mana, etc.
	...
	UNIT_FLAG_PASSIVE	                 = 0x00000200, // 10         512  A passive flag.
	...
	UNIT_FLAG_SILENCED                   = 0x00002000, // 14        8192  Mangos 1.12 / MangosZero claim this to be UNIT_FLAG_MOUNT?
...
enum DamageFlags
{
	/**
		...
		as we don't know if its handled bitwise or not 
		If someone wants to check with it, let me know
		- Shane											**/
	DAMAGE_FLAG_MELEE   = 1,
	DAMAGE_FLAG_HOLY	= 2,
	DAMAGE_FLAG_FIRE	= 4,
...
enum IMMUNE_STUFF {
/*			Name				Hex			Decimal			Bit		*/
	IMMUNE_CHARMED			= 0x000000001,	//1				01		
	IMMUNE_DISORIENTED		= 0x000000002,	//2				02		
	IMMUNE_DISARMED			= 0x000000004,	//4				03		
	IMMUNE_DISTRACED		= 0x000000008,	//8				04		
	IMMUNE_FLEEING			= 0x000000010,	//16			05		
	IMMUNE_CLUMSY			= 0x000000020,	//32			06		
	IMMUNE_ROOTED			= 0x000000040,	//64			07		
	IMMUNE_PACIFIED			= 0x000000080,	//128			08		
	IMMUNE_SILENCED			= 0x000000100,	//256			09		
	IMMUNE_ASLEEP			= 0x000000200,	//512			10		
    IMMUNE_ENSNARED			= 0x000000400,  //1024			11		
	IMMUNE_STUNNED			= 0x000000800,	//2048			12		
	IMMUNE_FROZEN			= 0x000001000,	//4096			13		
	IMMUNE_INCAPACIPATED	= 0x000002000,	//8192			14		
	IMMUNE_BLEEDING			= 0x000004000,	//16384			15		
	IMMUNE_HEALING			= 0x000008000,	//32768			16		
	IMMUNE_POLYMORPHED		= 0x000010000,	//65536			17		
	IMMUNE_BANISHED			= 0x000020000,	//131072		18		
	IMMUNE_SHIELDED			= 0x000040000,	//262144		19		
	IMMUNE_SHACKLED			= 0x000080000,	//524288		20		
	IMMUNE_MOUNTED			= 0x000100000,	//1048576		21		
	IMMUNE_SEDUCED			= 0x000200000,	//2097152		22		
	IMMUNE_TURNED			= 0x000400000,	//4194304		23		
	IMMUNE_HORRIFIED		= 0x000800000,	//8388608		24		
	IMMUNE_INVULNARABLE		= 0x001000000,	//16777216		25		
	IMMUNE_INTERRUPTED		= 0x002000000,	//33554432		26		
	IMMUNE_DAZED			= 0x004000000,	//67108864		27		
	IMMUNE_DISCOVERY 		= 0x008000000,	//134217728		28		
	IMMUNE_IMMUNE_SHIELD 	= 0x010000000,	//268435456		29		
	IMMUNE_SAPPED			= 0x020000000,	//536870912		30		
};
enum CUSTOM_IMMUNITIES
{
/*			Name			 Hex		Decimal	Bit		*/
	IMMUNE_SCHOOL_NORMAL = 0,			//1		1
	IMMUNE_SCHOOL_HOLY,					//2		2
	IMMUNE_SCHOOL_FIRE,					//4		3
	IMMUNE_SCHOOL_NATURE,				//8		4
	IMMUNE_SCHOOL_FROST,				//16	5
	IMMUNE_SCHOOL_SHADOW,				//32	6
	IMMUNE_SCHOOL_ARCANE,				//64	7
	IMMUNE_SHOUT,						//128	8
	IMMUNE_POISON,						//256	9
	IMMUNE_DOT,							//512	10
	IMMUNE_DISEASE,						//1024	12
	IMMUNE_TAUNT,						//2048	13
	IMMUNE_DEBUFF,						//4096	14
};
...
public:
	...
	CHRONO_INLINE bool IsInCombat() { return m_lastStatus; }				// checks if we are in combat or not.

	void OnRemoveFromWorld();										// called when we are removed from world, kills all references to us.
	
	CHRONO_INLINE void Vanished()
	{
		DamageMap.clear();
		ClearAttackers();
		ClearHealers();
	...
	CHRONO_INLINE void PutInCombat()
	{
		m_lastStatus = 1;
	}

	CHRONO_INLINE const uint64& GetPrimaryAttackTarget() { return m_primaryAttackTarget; }
	CHRONO_INLINE void SetUnit(Unit * p) { m_Unit = p; }
	void TryToClearAttackTargets();									// for pvp timeout
	void RewardHonorForAttackers();									// not really the right place to do this but who cares
...
/*	typedef struct { const char *name; int(*mfunc)(lua_State*,Unit*); } RegType;
	...
	CHRONO_INLINE void SetDuelWield(bool enabled) { m_duelWield = enabled; }
  ...
  //void StrikeWithAbility( Unit* pVictim, Spell* spell, uint32 addspelldmg, uint32 weapon_damage_type );
	...
	CHRONO_INLINE void addStateFlag(uint32 f) { m_state |= f; };
	CHRONO_INLINE bool hasStateFlag(uint32 f) { return (m_state & f ? true : false); }
	CHRONO_INLINE void clearStateFlag(uint32 f) { m_state &= ~f; };

	/// Stats
	CHRONO_INLINE uint32 getLevel() { return m_uint32Values[ UNIT_FIELD_LEVEL ]; };
	CHRONO_INLINE uint8 getRace() { return GetByte(UNIT_FIELD_BYTES_0,0); }
	CHRONO_INLINE uint8 getClass() { return GetByte(UNIT_FIELD_BYTES_0,1); }
	CHRONO_INLINE void setRace(uint8 race) { SetByte(UNIT_FIELD_BYTES_0,0,race); }
	CHRONO_INLINE void setClass(uint8 class_) { SetByte(UNIT_FIELD_BYTES_0,1, class_ ); }
	CHRONO_INLINE uint32 getClassMask() { return 1 << (getClass() - 1); }
	CHRONO_INLINE uint32 getRaceMask() { return 1 << (getRace() - 1); }
	CHRONO_INLINE uint8 getGender() { return GetByte(UNIT_FIELD_BYTES_0,2); }
	CHRONO_INLINE void setGender(uint8 gender) { SetByte(UNIT_FIELD_BYTES_0,2,gender); }
	CHRONO_INLINE uint8 getStandState() { return ((uint8)m_uint32Values[UNIT_FIELD_BYTES_1]); }
...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	CHRONO_INLINE void setHRegenTimer(uint32 time) {m_H_regenTimer = time; }
	CHRONO_INLINE void setPRegenTimer(uint32 time) {m_P_regenTimer = time; }
	CHRONO_INLINE void DelayPowerRegeneration(uint32 time) { m_P_regenTimer = time; if (!m_interruptedRegenTime) m_interruptedRegenTime = 2000; }
	void DeMorph();
	...
	// Stealth  
	CHRONO_INLINE int32 GetStealthLevel() { return m_stealthLevel; }
	CHRONO_INLINE int32 GetStealthDetectBonus() { return m_stealthDetectBonus; }
	CHRONO_INLINE void SetStealth(uint32 id) { m_stealth = id; }
	CHRONO_INLINE bool IsStealth() { return (m_stealth!=0 ? true : false); }
	float detectRange;
	...
	/// Combat / Death Status
	CHRONO_INLINE bool isAlive() { return m_deathState == ALIVE; };
	CHRONO_INLINE bool isDead() { return  m_deathState !=ALIVE; };
...
//	std::map<uint32,ProcTriggerSpellOnSpellList> m_procSpellonSpell; //index is namehash
	...
	void WipeHateList();
	void WipeTargetList();
	CHRONO_INLINE void setAItoUse(bool value){m_useAI = value;}
	...
	// DK:Affect
	CHRONO_INLINE uint32 IsPacified() { return m_auracount[SPELL_AURA_MOD_PACIFY]; }
	CHRONO_INLINE uint32 IsStunned() { return m_auracount[SPELL_AURA_MOD_STUN]; }
	CHRONO_INLINE uint32 IsFeared() { return m_auracount[SPELL_AURA_MOD_FEAR]; }
	CHRONO_INLINE uint32 GetResistChanceMod() { return m_resistChance; }
	CHRONO_INLINE void SetResistChanceMod(uint32 amount) { m_resistChance=amount; }
	
	CHRONO_INLINE uint16 HasNoInterrupt() { return m_noInterrupt; }
	...
	uint32 dispels[10];
	bool trackStealth;
	map <uint32, uint32> MechanicsDispels;
	...
	void EmoteExpire();
	CHRONO_INLINE void setEmoteState(uint8 emote) { m_emoteState = emote; };
	CHRONO_INLINE uint32 GetOldEmote() { return m_oldEmote; }
	...
	CHRONO_INLINE StandState GetStandState()
	{
	...
	CHRONO_INLINE int GetHealthPct() { return (int)(GetUInt32Value(UNIT_FIELD_HEALTH) * 100 / GetUInt32Value(UNIT_FIELD_MAXHEALTH)); };
    CHRONO_INLINE void SetHealthPct(uint32 val) { if (val>0) SetUInt32Value(UNIT_FIELD_HEALTH,float2int32(val*0.01f*GetUInt32Value(UNIT_FIELD_MAXHEALTH))); };
	CHRONO_INLINE int GetManaPct() { return (int)(GetUInt32Value(UNIT_FIELD_POWER1) * 100 / GetUInt32Value(UNIT_FIELD_MAXPOWER1)); };
		
	uint32 GetResistance(uint32 type);	
	
	//Pet
	CHRONO_INLINE void SetIsPet(bool chck) { m_isPet = chck; }
	...
	CHRONO_INLINE Spell * GetCurrentSpell(){return m_currentSpell;}
	CHRONO_INLINE void SetCurrentSpell(Spell* cSpell) { m_currentSpell = cSpell; }

	uint32 m_CombatUpdateTimer;

	CHRONO_INLINE void setcanperry(bool newstatus){can_parry=newstatus;}
	...
	map <uint32, uint32> CustomImmunityList;
	...
	CHRONO_INLINE uint32 GetPowerType(){ return (GetUInt32Value(UNIT_FIELD_BYTES_0)>> 24);}
	...
	uint16 m_auracount[192]; //needs to be set to 192
	...
	CHRONO_INLINE uint32 GetCharmTempVal() { return m_charmtemp; }
	CHRONO_INLINE void SetCharmTempVal(uint32 val) { m_charmtemp = val; }
	set<uint32> m_SpellList;

	CHRONO_INLINE void DisableAI() { m_useAI = false; }
	CHRONO_INLINE void EnableAI() { m_useAI = true; }

	CHRONO_INLINE void SetPowerType(uint8 type)
	{
		SetByte(UNIT_FIELD_BYTES_0,3,type);
	}

	CHRONO_INLINE bool IsSpiritHealer()
	{
		switch(GetEntry())
		{
	...
	/* immunity functions */
	void AddImmuneEffect(uint8 effect);
	bool IsImmuneFor(uint8 effect);
	void RemoveImmuneEffect(uint8 effect);
	
	void AddCustomImmuneEffect(uint8 effect);
	bool IsImmuneForCustomEffect(uint8 effect);
	void RemoveCustomEffect(uint8 effect);
