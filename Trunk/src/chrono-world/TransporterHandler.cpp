...
bool Transporter::GenerateWaypoints()
{
    ...
    //        sLog.Notice("", "%f, %f, %f, %f, %f, %f, %f", keyFrames[i].x, keyFrames[i].y, keyFrames[i].distUntilStop, keyFrames[i].distSinceStop, keyFrames[i].distFromPrev, keyFrames[i].tFrom, keyFrames[i].tTo);
    //    }
    ...
    for (size_t i = 0; i < keyFrames.size() - 1; i++)       //
    {
        ...
        if (((d < keyFrames[i + 1].distFromPrev) && (tTo > 0)))
        {
            while ((d < keyFrames[i + 1].distFromPrev) && (tTo > 0))
            {
                ...
                if (d > 0)
                {
                    ...
                    //                    sLog.Notice("", "T: %d, D: %f, x: %f, y: %f, z: %f", t, d, newX, newY, newZ);
        ...
        //        sLog.Notice("", "T: %d, x: %f, y: %f, z: %f, t:%d", t, pos.x, pos.y, pos.z, teleport);
