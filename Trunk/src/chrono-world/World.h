...
public:
	...
	CHRONO_INLINE size_t GetSessionCount() const { return m_sessions.size(); }
	uint32 GetNonGmSessionCount();
	CHRONO_INLINE size_t GetQueueCount() { return mQueuedSessions.size(); }
	void GetStats(uint32 * GMCount, float * AverageLatency);

	CHRONO_INLINE uint32 GetPlayerLimit() const { return m_playerLimit; }
	void SetPlayerLimit(uint32 limit) { m_playerLimit = limit; }

	CHRONO_INLINE bool getAllowMovement() const { return m_allowMovement; }
	void SetAllowMovement(bool allow) { m_allowMovement = allow; }
	CHRONO_INLINE bool getGMTicketStatus() { return m_gmTicketSystem; };
	...
	CHRONO_INLINE std::string getGmClientChannel() { return GmClientChannel; }

	void SetMotd(const char *motd) { m_motd = motd; }
	CHRONO_INLINE const char* GetMotd() const { return m_motd.c_str(); }

	CHRONO_INLINE time_t GetGameTime() const { return m_gameTime; }
	...
	CHRONO_INLINE void SetStartTime(uint32 val) { m_StartTime = val; }
	CHRONO_INLINE uint32 GetUptime(void) { return (uint32)UNIXTIME - m_StartTime; }
	CHRONO_INLINE uint32 GetStartTime(void) { return m_StartTime; }
	...
	CHRONO_INLINE void setRate(int index,float value)
	{
		regen_values[index]=value;
	}

	CHRONO_INLINE float getRate(int index)
	{
		return regen_values[index];
	}
	
	CHRONO_INLINE uint32 getIntRate(int index)
	{
		return int_rates[index];
	}

	CHRONO_INLINE void setIntRate(int index, uint32 value)
	{
	...
	CHRONO_INLINE uint32 GetTimeOut(){return TimeOut;}
	...
	void SetDateLastMaintenanceDay(uint32 Day) {m_LastMaintenanceDate = Day;}
	void SetDateNextMaintenanceDay(uint32 Day) {m_NextMaintenanceDate = Day;}
	...
	void FlushRankPoints(uint32 DateTop);
        ...
        uint32 GetDateLastMaintenanceDay() const
        {
            ...
            uint32 mDay  = 4; // getConfig(CONFIG_UINT32_MAINTENANCE_DAY);
...
protected:
	...
	uint32 m_MaintainceTimer;
	uint32 m_NextMaintenanceDate;
	uint32 m_LastMaintenanceDate;
