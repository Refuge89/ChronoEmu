...
void ObjectMgr::LoadAIThreatToSpellId()
{
	...
	do
	{
		...
		{
			Log.Warning("AIThreatSpell", "Cannot apply to spell %u; spell is nonexistant.");
		}
	} while( result->NextRow() );
...
{
	QueryResult * result = CharacterDatabase.Query("SELECT * FROM playeritems WHERE guid = %u", Chrono::Util::GUID_LOPART(guid));
	Item * pReturn = 0;
...
{
	QueryResult * result = CharacterDatabase.Query("SELECT * FROM playeritems_external WHERE guid = %u", Chrono::Util::GUID_LOPART(guid));
	Item * pReturn = 0;
	...
	do 
	{
		...
		else
		{
			do
			{
				...
				if(fields2[6].GetUInt32() == 0)
				{
					SpellEntry * tempspell = dbcSpell.LookupEntryForced( ts.pLearnSpell->EffectTriggerSpell[0] );
					if(tempspell && tempspell->baseLevel > 0)
						WorldDatabase.Execute("UPDATE trainer_spells SET reqlevel='%i' WHERE entry='%i' AND learn_spell='%i'",tempspell->baseLevel, entry ,ts.pLearnSpell->Id);
...
void ObjectMgr::GenerateLevelUpInfo()
{
	...
	for(uint32 Class = WARRIOR; Class <= DRUID; ++Class)
	{
		...
		for(uint32 Race = RACE_HUMAN; Race <= RACE_TROLL; ++Race )
		{
			...
			*lvl = lastlvl;
			...
			for(uint32 Level = 2; Level < MaxLevel; ++Level)
			{
				...
				{
				case WARRIOR:
					if(Level<14)TotalHealthGain+=19;
					else if(Level<37) TotalHealthGain+=Level+6;
				...
				case WARLOCK:
					...
					break;
				case PALADIN:
					if(Level < 15)TotalHealthGain+=18;
					else TotalHealthGain+=Level+4;

					if(Level<24)TotalManaGain+=Level+17;
				...
				case PRIEST:
					...
					else if(Level <33)TotalManaGain+=Level+37;
...
bool ObjectMgr::HandleInstanceReputationModifiers(Player * pPlayer, Unit * pVictim)
{
	...
	is_boss = 0;//TO_CREATURE( pVictim )->GetCreatureName() ? ((Creature*)pVictim)->GetCreatureName()->Rank : 0;
...
void ObjectMgr::LoadStandingList(uint32 dateBegin)
{
    ...
    if (result)
	{
		...
		side = GetPlayerTeamByGUID(guid);

		kills=0;
		// kills count with victim setted ( not zero value )
		result2 = CharacterDatabase.Query("SELECT COUNT(*) FROM character_honor_cp WHERE guid = %u AND victim>0 AND TYPE = %u AND date BETWEEN %u AND %u",guid,HONORABLE,dateBegin,dateBegin+7);
		if (result2)
			kills = result2->Fetch()->GetUInt32();

		// you need to reach CONFIG_UINT32_MIN_HONOR_KILLS to be added in standing list
		if (kills < 15) //you need 15 kills a week otherwise it won't be proccessed
			continue;
...
uint32 ObjectMgr::GetPlayerTeamByGUID(uint64 guid)
{
	QueryResult *result = CharacterDatabase.Query("SELECT race FROM characters WHERE guid = '%u'", guid);
	
	if(result)
	{
		Field * fields = result->Fetch();
		uint8 race = fields[0].GetUInt32();
		uint8 team = 0;
		delete result;
	...
	//else TODO: cleanup when it happens
	return 0;
}
void ObjectMgr::DistributeRankPoints(uint32 team, uint32 dateBegin , bool flush /*false*/)
{
    ...
    {
        RP = 0;
        result = CharacterDatabase.Query("SELECT stored_honor_rating,stored_honorable_kills FROM characters WHERE guid = %u ",itr->guid);
        if (!result)
        ...
        if (flush)
        {
            CharacterDatabase.Execute("DELETE FROM character_honor_cp WHERE guid = %u AND TYPE = %u AND date BETWEEN %u AND %u",itr->guid,HONORABLE,dateBegin,dateBegin+7);
            CharacterDatabase.Execute("UPDATE characters SET stored_honor_rating = %f , stored_honorable_kills = %u WHERE guid = %u",(RP+itr->rpEarning),HK+itr->honorKills,itr->guid);
			
			/* update player */

			Player* tempplayer = GetPlayer(itr->guid);

			if(tempplayer)
			{
				tempplayer->m_stored_honor = RP+itr->rpEarning;
				tempplayer->m_stored_honorableKills = HK+itr->honorKills;
				tempplayer->UpdateHonor();
			}
        }
    }
...
{
    HonorStandingList standingList = objmgr.GetStandingListBySide(side);
    uint32 pos = 1;
...
void ObjectMgr::loadSavedVariables()
{
	sWorld.SetDateLastMaintenanceDay(sWorld.GetDateLastMaintenanceDay());
	sWorld.SetDateNextMaintenanceDay(sWorld.GetDateToday() + 7 - ( sWorld.GetDateToday() - sWorld.GetDateLastMaintenanceDay() ) );

	//use db data for this so, we don't mis a day because of a crash
}
