...
World::World()
{
	...
	m_CEThreshold = 10000;
	m_MaintainceTimer = 120000; //update timer after 2 minutes
}
...
// ------------------------------------------------------------------------------------------------
	...
	CBattlegroundManager * battlemaster = new CBattlegroundManager;
	battlemaster->Init();
...
void World::Update(time_t diff)
{
	...
	if (m_MaintainceTimer < diff)
    {
		/*sLog.outError("GetDateToday() = %i",GetDateToday());
		sLog.outError("m_NextMaintenanceDate = %i",m_NextMaintenanceDate);*/
		if (GetDateToday() >= m_NextMaintenanceDate)
        {
			//sLog.outError("maintenance start!");
			FlushRankPoints(m_LastMaintenanceDate);
			objmgr.LoadStandingList();
			m_LastMaintenanceDate = GetDateToday();
			m_NextMaintenanceDate += 7;
        }
        m_MaintainceTimer = 30000; // check 120 minutes
    }
    else
        m_MaintainceTimer -= diff;
...
void World::Rehash(bool load)
{
	...
	{
		#ifdef WIN32
		Config.MainConfig.SetSource("chrono-world.conf", true);
		#else
		Config.MainConfig.SetSource((char*)CONFDIR "/chrono-world.conf", true);
		#endif
	}
...
void World::FlushRankPoints(uint32 DateTop)
{
	QueryResult * result = CharacterDatabase.Query("SELECT date FROM character_honor_cp WHERE TYPE = %u AND date <= %u GROUP BY date ORDER BY date DESC",HONORABLE,DateTop);
	if(result)
	{
		uint32 date;
        bool flush;
        ...
        {
			objmgr.LoadStandingList(WeekBegin);

            flush = WeekBegin < DateTop - 7; // flush only with date < lastweek

            objmgr.DistributeRankPoints(ALLIANCE,WeekBegin,flush);
            objmgr.DistributeRankPoints(HORDE,WeekBegin,flush);
