...
bool ChatHandler::HandleAddTrainerSpellCommand( const char * args, WorldSession * m_session )
{
	...
	WorldDatabase.Execute("INSERT INTO trainer_spells VALUES(%u, %u, %u, %u, %u, %u, %u, %u)", 
		pCreature->GetEntry(), pSpell->Id, cost, reqspell, (int)0, (int)0, reqlevel, delspell);
...
bool ChatHandler::HandleDeleteCommand(const char* args, WorldSession *m_session)
{
	...
	if( unit->m_spawn != nullptr && !m_session->CanUseCommand('z') )
	{
	...
	if(unit->m_spawn)
	{
		...
		if(cellx <= _sizeX && celly <= _sizeY && unitMgr != nullptr)
		{
...
bool ChatHandler::HandleItemRemoveCommand(const char* args, WorldSession *m_session)
{
	...
	if(slot != -1)
	{
		uint32 guidlow = Chrono::Util::GUID_LOPART(guid);
...
bool ChatHandler::HandleNPCFlagCommand(const char* args, WorldSession *m_session)
{
	...
	pCreature->SetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_02, npcFlags);
...
bool ChatHandler::HandleCastSpellCommand(const char* args, WorldSession *m_session)
{
	...
	Spell *sp = new Spell(caster, spellentry, false, nullptr);
	if(!sp)
	{
...
bool ChatHandler::HandleGOSelect(const char *args, WorldSession *m_session)
{
	...
	if(args)
	{
		if(args[0] == '1')
		{
			...
			for(;;Itr++)
			{
				if(Itr == Itr2 && GObj == nullptr && bUseNext)
...
bool ChatHandler::HandleGODelete(const char *args, WorldSession *m_session)
{
  ...
  /*  std::stringstream sstext;
	...
	m_session->GetPlayer()->m_GM_SelectedGO = nullptr;
...
bool ChatHandler::HandleAddAIAgentCommand(const char* args, WorldSession *m_session)
{
	...
	if(!agent)
		return false;
	char* procEvent = strtok(nullptr, " ");
	if(!procEvent)
		return false;
	char* procChance = strtok(nullptr, " ");
	if(!procChance)
		return false;
	char* procCount = strtok(nullptr, " ");
	if(!procCount)
		return false;
	char* spellId = strtok(nullptr, " ");
	if(!spellId)
		return false;
	char* spellType = strtok(nullptr, " ");
	if(!spellType)
		return false;
	char* spelltargetType = strtok(nullptr, " ");
	if(!spelltargetType)
		return false;
	char* spellCooldown = strtok(nullptr, " ");
	if(!spellCooldown)
		return false;
	char* floatMisc1 = strtok(nullptr, " ");
	if(!floatMisc1)
		return false;
	char* Misc2 = strtok(nullptr, " ");
	if(!Misc2)
		return false;
...
bool ChatHandler::HandleNpcComeCommand(const char* args, WorldSession* m_session)
{
	...
	pCreature->SetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_03, npcFlags);
	SystemMessage(m_session, "Value saved, you may need to rejoin or clean your client cache.");
