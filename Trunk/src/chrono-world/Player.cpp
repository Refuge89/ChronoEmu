...
Player::Player( uint32 guid ) : m_mailBox(guid)
{
	...
	m_AutoShotSpell			= nullptr;

	m_AttackMsgTimer		= 0;

	timed_quest_slot		= 0;
	m_GM_SelectedGO			= nullptr;
...
Player::~Player ( )
{
	...
	// Once again, let's clear for memeory sake
	if(myCorpse)
		myCorpse = NULLCORPSE;
...
CHRONO_INLINE uint32 GetSpellForLanguage(uint32 SkillID)
{
	switch(SkillID)
	{
...
void Player::_EventAttack( bool offhand )
{
	...
	else
	{
		...
		{ 
			...
			SetOnMeleeSpell(0);
			Spell *spell = new Spell(this,spellInfo,true,nullptr);
...
void Player::_EventCharmAttack()
{
	...
	else
	{
		...
		else
		{
			...
			{
				m_CurrentCharm->Strike( pVictim, MELEE, nullptr, 0, 0, 0, false, false );
			} 
			else 
			{ 
				SpellEntry *spellInfo = dbcSpell.LookupEntry(m_CurrentCharm->GetOnMeleeSpell());
				m_CurrentCharm->SetOnMeleeSpell(0);
				Spell *spell = new Spell(m_CurrentCharm,spellInfo,true,nullptr);
				SpellCastTargets targets;
				targets.m_unitTarget = GetSelection();
				spell->prepare(&targets);
...
void Player::_EventExploration()
{
	...
	if(AreaId != m_AreaID)
	{
		...
		UpdatePvPArea();
		if(GetGroup())
            GetGroup()->UpdateOutOfRangePlayer(this, 128, true, nullptr);
	}
...
void Player::BuildEnumData( QueryResult * result, WorldPacket * p_data )
{
	...
	*p_data << uint64(fields[0].GetUInt32());
    *p_data << fields[7].GetString();                       // name
    *p_data << uint8(fields[2].GetUInt8());                 // race
    *p_data << uint8(fields[3].GetUInt8());                 // class
    ...
    *p_data << fields[7].GetUInt32();                       // guild id
    ...
    if(playerFlags & PLAYER_FLAG_DEATH_WORLD_ENABLE)
        char_flags |= CHARACTER_FLAG_GHOST;

	*p_data << uint32(char_flags);
	...
	*p_data << (uint32)petFamily;
    ...
    {
        if (GetItemInterface()->GetInventoryItem(i) != nullptr)
        {
...
void Player::GiveXP(uint32 xp, const uint64 &guid, bool allowbonus)
{
	...
	HandleProc(PROC_ON_KILL, this, nullptr);
	m_procCounter = 0;
}
...
void Player::InitVisibleUpdateBits()
{
	...
	Player::m_visibleUpdateMask.SetBit(UNIT_FIELD_FLAGS + 1); // Possibly no good
...
void Player::SaveToDB(bool bNewCharacter /* =false */)
{
	...
	sLog.outError("player_flags = %i",player_flags);
	{
	...
	// [INFINITE] This isn't used in 1.12. Bad Update Fields. Needs a fix -- Shane
	"0,"
	"0,"
...
//	ss << LoadAuras;
	...
	ss << "', ";
	ss << uint32(GetHonorRankInfo().rank)  << ", ";
	ss << m_standing_pos << ", " << m_stored_honor << ", " << m_stored_dishonorableKills << ", ";
...
bool Player::canCast(SpellEntry *m_spellInfo)
{
	if (m_spellInfo->EquippedItemClass != -1)
	{
		if(this->GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND))
		{
			...
			{
				if (m_spellInfo->EquippedItemSubClass != -1)
				{
...
#define get_next_field fields[field_index++]
	...
	{
		Transporter * t = objmgr.GetTransporter(Chrono::Util::GUID_LOPART(m_TransporterGUID));
...
//#undef _language_fix_
    ...
    m_highest_rank.rank        = get_next_field.GetUInt32();
    m_highest_rank  = MaNGOS::Honor::CalculateRankInfo(m_highest_rank);
    m_standing_pos             = get_next_field.GetUInt32();
    m_stored_honor             = get_next_field.GetUInt32();
    m_stored_dishonorableKills = get_next_field.GetUInt32();
    m_stored_honorableKills    = get_next_field.GetUInt32();

    iInstanceType = get_next_field.GetUInt32();

	QueryResult *result2 = CharacterDatabase.Query("SELECT victim_type,victim,honor,date,type FROM character_honor_cp WHERE guid = '%u'",GetGUID());

	_LoadHonorCP(result2);
	...
	if( results[8].result != nullptr )			// this query is "who are our friends?"
	{
		...
		do 
		{
			...
			if( strlen( fields[1].GetString() ) )
				...
				m_friends.insert( make_pair( fields[0].GetUInt32(), (char*)nullptr) );
...
void Player::_ApplyItemMods(Item* item, int8 slot, bool apply, bool justdrokedown /* = false */, bool skip_stat_apply /* = false  */)
{
			...
			if( !set->RequiredSkillID || ( _GetSkillLineCurrent( set->RequiredSkillID, true ) >= set->RequiredSkillAmt ) )
			{
				for( uint32 x=0;x<8;x++)
				{
					...
					{//cast new spell
						...
						Spell * spell = new Spell( this, info, true, nullptr );
						SpellCastTargets targets;
	...
	if( apply )
	{
		...
		for( int k = 0; k < 5; k++ )
		{
			...
			if( item->GetProto()->Spells[k].Trigger == 1 )
			{
				...
				Spell *spell = new Spell( this, spells ,true, nullptr );
				SpellCastTargets targets;
				targets.m_unitTarget = this->GetGUID();
	...
	{
		// Remove certain "enchantments" that are not actually enchantments
		for( uint32 x = 0 ; x <= MAX_POSITIVE_AURAS ; x ++ )
		{
			if( m_auras[x] )
			{
				...
				if( m_auras[x]->GetSpellProto()->EquippedItemClass > -1 && m_auras[x]->GetSpellProto()->EquippedItemSubClass > -1 )
				{
					if( item->GetProto()->Class == m_auras[x]->GetSpellProto()->EquippedItemClass && ( 1 << item->GetProto()->SubClass ) && m_auras[x]->GetSpellProto()->EquippedItemSubClass > -1 ) // fucking bits
						m_auras[x]->Remove();
				}
			}
		}
...
void Player::BuildPlayerRepop()
{
	...
	if(getRace()==RACE_NIGHTELF)
	{
		...
		Spell*sp=new Spell(this,inf,true,nullptr);
		sp->prepare(&tgt);
		inf=dbcSpell.LookupEntry(9036);
		sp=new Spell(this,inf,true,nullptr);
		sp->prepare(&tgt);
	}
	else
	{
...
void Player::RepopRequestedPlayer()
{
	...
	if( m_CurrentTransporter != nullptr )
	{
	...
	if( pMapinfo != nullptr )
	{
		...
		else if( pMapinfo->type == INSTANCE_NULL || pMapinfo->type == INSTANCE_PVP )
		{
			RepopAtGraveyard( GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId() );
		}
		else
...
//
// References WorldSafeLocs.dbc
// Proper Implementation without the need for a graveyards table.
// 
// Ported from Soulstone SVN & I submitted it to Ascemu as S7urp33.
//
void Player::RepopAtGraveyard(float ox, float oy, float oz, uint32 map_id)
{
	// No factions are required, you will port to the nearest graveyard.
	float closestX, closestY, closestZ;
	float mx = ox, my = oy, last_distance = 9e10f;

	for (uint32 x = 0; x < dbcworldsafelocs.GetNumRows(); x++)
	{
		WorldSafeLocs *g = dbcworldsafelocs.LookupEntry(x);                  
	...
	// Teleport the player to a proper Graveyard.
	SafeTeleport(map_id, 0, LocationVector(closestX, closestY, closestZ, 0));
}
...
{
	// [INFINITE] Needs Re-write for 1.12 --Shane
	uint32 pClass = (uint32)getClass();
	uint32 pLevel = (getLevel() > 60) ? 60 : getLevel();
	...
	if( it != nullptr && it->GetProto()->InventoryType == INVTYPE_SHIELD )
	{
	...
	{
		if( itr->second.wclass == ( uint32 )-1 || ( tItemMelee != nullptr && ( 1 << tItemMelee->GetProto()->SubClass & itr->second.subclass ) ) )
		{
			melee_bonus += itr->second.value;
		}
		if( itr->second.wclass == ( uint32 )-1 || ( tItemRanged != nullptr && ( 1 << tItemRanged->GetProto()->SubClass & itr->second.subclass ) ) )
		{
	...
	spellcritperc+=CalcRating( PLAYER_RATING_MODIFIER_SPELL_CRIT );
	
	//UpdateChanceFields();
}

// [INFINITE] Can this be used to update Spell Damage / Healing bonuses? -- Shane
...
void Player::UpdateAttackSpeed()
{
	...
	if( weap != nullptr && weap->GetProto()->Class == 2 )// 2 is a weapon
	{
		speed = weap->GetProto()->Delay;
...
{   
	...
	int32 stat_bonus = GetUInt32Value( PLAYER_FIELD_POSSTAT2 ) - GetUInt32Value( PLAYER_FIELD_NEGSTAT2 );
	if ( stat_bonus < 0 )
	...
	if( cl != WARRIOR && cl != ROGUE )
	{
		...
		stat_bonus = GetUInt32Value( PLAYER_FIELD_POSSTAT3 ) - GetUInt32Value( PLAYER_FIELD_NEGSTAT3 );
		if ( stat_bonus < 0 )
	...
	// [INFINITE] Not sure the exact calculations for this, 
	// but Is this possibly right for 1.12? --Shane
	////////////////////RATINGS STUFF//////////////////////
/*
	// Shield Block
	Item* shield = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_OFFHAND);
	if( shield != nullptr && shield->GetProto()->InventoryType == INVTYPE_SHIELD )
	{
...
{
	if(GetUInt32Value(UNIT_FIELD_LEVEL) >= 60)		// Save CPU, don't waste time on this if you've reached max_level
		amount = 0;
...
void Player::AddCalculatedRestXP(uint32 seconds)
{
	...
	if (m_isResting) // if we are at a resting area rest_XP goes 4 times faster (making it 1 bubble every 2 hrs)
		bubblerate = bubblerate * 4;
	
	// One bubble (5% of xp_to_level) for every 8 hours logged out.
	// if multiplier RestXP (from chrono.config) is f.e 2, you only need 4hrs/bubble.
...
void Player::UpdateRestState()
{
	...
	SetUInt32Value(PLAYER_REST_STATE_EXPERIENCE, m_restAmount);
}
...
bool Player::CanSee(Object* obj) // * Invisibility & Stealth Detection - Partha *
{
	...
	//if( !PhasedCanInteract(obj) ) //PhasedCanInteract is from our 3.3 core
		//return false;
		...
		case TYPEID_PLAYER:
			{
				...
				//we may want to copy m_invisible over from our 3.3 if neccessary
...
void Player::AddInRangeObject(Object* pObj)
{
	...
	if (m_CurrentTaxiPath && (pObj->GetTypeId() == TYPEID_PLAYER))
	{
		...
		if (ntime > m_taxi_ride_time)
			m_CurrentTaxiPath->SendMoveForTime( this, TO_PLAYER( pObj ), ntime - m_taxi_ride_time);
		/*else
			m_CurrentTaxiPath->SendMoveForTime( this, TO_PLAYER( pObj ), m_taxi_ride_time - ntime);*/
	}
	...
	if(pObj->GetTypeId() == TYPEID_UNIT)
	{
		...
		//sEventMgr.AddEvent( TO_CREATURE( pObj )->GetAIInterface(), &AIInterface::SendCurrentMove, this->GetGUID(), EVENT_UNIT_SENDMOVE, 200, 1);
...
{	
	...
	{ 
		...
		slottype = 0;
		if(m_Group != nullptr && loot_type < 2)
		{
			switch(loot_method)
			{
		...
		if(iter->iRandomProperty != nullptr)
		{
			data << uint32(0);
			data << uint32(0);
		}
		...
		if(slottype == 1)
		{
			if(iter->roll == nullptr && !iter->passed)
			{
				int32 ipid = 0;
				uint32 factor=0;
				...
				{
					iter->roll = new LootRoll(60000, (m_Group != nullptr ? m_Group->MemberCount() : 1),  guid, x, iter->item.itemproto->ItemId, factor, uint32(ipid), GetMapMgr());
...
int32 Player::CanShootRangedWeapon( uint32 spellid, Unit* target, bool autoshot )
{
	...
	// I don't think this is needed since the "auto shot" ability is a spell.
	...
	{
		if( itm != nullptr ) // no need for this
			if( itm->GetProto() )
...
void Player::EventRepeatSpell()
{
	...
	{		
		...
		Spell* sp = new Spell( this, m_AutoShotSpell, true, nullptr );
		SpellCastTargets tgt;
...
void Player::CalcResistance(uint32 type)
{
	...
	SetUInt32Value(PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE+type,pos);
	SetUInt32Value(PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE+type,neg);
			...
			if((*itr)->GetUInt32Value(GAMEOBJECT_TYPE_ID) == GAMEOBJECT_TYPE_QUESTGIVER)
			{
				if(((GameObject*)(*itr))->m_quests)
				{
					if(((GameObject*)(*itr))->m_quests->size() > 0)
					{
						...
						{
							uint32 status = sQuestMgr.CalcQuestStatus(nullptr, this, (*itr2)->qst, (*itr2)->type, false);
							if(status == QMGR_QUEST_CHAT || status == QMGR_QUEST_AVAILABLE || status == QMGR_QUEST_FINISHED)
							{
								// Activate gameobject
...
void Player::CalcStat(uint32 type)
{
	...
	pos += ( res * TO_PLAYER( this )->TotalStatModPctPos[type] ) / 100;
	neg += ( res * TO_PLAYER( this )->TotalStatModPctNeg[type] ) / 100;
	res = pos + BaseStats[type] - neg;
	if( res <= 0 )
		res = 1;

	SetUInt32Value( PLAYER_FIELD_POSSTAT0 + type, pos );
	SetUInt32Value( PLAYER_FIELD_NEGSTAT0 + type, neg );
...
void Player::RegenerateMana(bool is_interrupted)
{
	...
	float addvalue = 0;
	float Spirit = GetUInt32Value(UNIT_FIELD_STAT4);
    uint8 Class = getClass();

	switch (Class)
	{
		case DRUID:   addvalue = (Spirit/5 + 15);   break;
		case HUNTER:  addvalue = (Spirit/5 + 15);    break;
		case MAGE:    addvalue = (Spirit/4 + 12.5); break;
		case PALADIN: addvalue = (Spirit/5 + 15);   break;
		case PRIEST:  addvalue = (Spirit/4 + 12.5); break;
		case SHAMAN:  addvalue = (Spirit/5 + 17);   break;
		case WARLOCK: addvalue = (Spirit/5 + 15);   break;
	}

	if(is_interrupted)
	{
		addvalue *= (m_ModInterrMRegenPCT / 100);
	}
	
	addvalue *= PctPowerRegenModifier[POWER_TYPE_MANA]; //mp5
	addvalue *= sWorld.getRate(RATE_POWER1);

	if(cur + addvalue > mm)
		addvalue = mm - cur; //we can't have higher then max ofc.

	ModUnsigned32Value(UNIT_FIELD_POWER1, float2int32(addvalue));
}
...
void Player::RegenerateHealth( bool inCombat )
{
    ...
    switch (Class)
    {
        case DRUID:   addvalue = (Spirit*0.11 + 1);    break;
        case HUNTER:  addvalue = (Spirit*0.43 - 5.5);  break;
        case MAGE:    addvalue = (Spirit*0.11 + 1);    break;
        case PALADIN: addvalue = (Spirit*0.25);        break;
        case PRIEST:  addvalue = (Spirit*0.15 + 1.4);  break;
        case ROGUE:   addvalue = (Spirit*0.84 - 13);   break;
        case SHAMAN:  addvalue = (Spirit*0.28 - 3.6);  break;
        case WARLOCK: addvalue = (Spirit*0.12 + 1.5);  break;
        case WARRIOR: addvalue = (Spirit*1.26 - 22.6); break;
    }

	addvalue = addvalue * sWorld.getRate(RATE_HEALTH);

	if(cur + addvalue > mh)
		addvalue = mh - cur; //we can't have higher then max ofc.

	// ?
	if(inCombat)
		addvalue = 0;

	ModUnsigned32Value(UNIT_FIELD_HEALTH, float2int32(addvalue));
}
...
{
	GetSession()->OutPacket(SMSG_GOSSIP_COMPLETE, 0, nullptr);
	CleanupGossipMenu();
}
...
void Player::SendTradeUpdate()
{
	...
	for(uint8 Index = 0; Index < 7; ++Index)
	{
		...
		if(pItem != 0)
		{
			...
			data << pItem->GetChargesLeft();  // Spell Charges	OK

			data << uint32(0);									  		// Random Suffix?
...
void Player::BuildFlagUpdateForNonGroupSet(uint32 index, uint32 flag)
{
    ...
    for (Object::InRangeSet::iterator iter = GetInRangeSetBegin(); iter != GetInRangeSetEnd();)
	{
        ...
        if(curObj->IsPlayer())
        {
            Group* pGroup = TO_PLAYER( curObj )->GetGroup();
            if( pGroup != nullptr && pGroup == GetGroup())
            {
...
*/
		...
		this->AddAura(a, nullptr);		//FIXME: must save amt,pos/neg
...
//			this->AddAuraVisual((*i).id,count_appearence-1,a->IsPositive());
	...
	// useless logon spell
	/*Spell *logonspell = new Spell(this, dbcSpell.LookupEntry(836), false, nullptr);
	...
	loginauras.clear(); // Probably not the best place, but this entire function is fail.
}
...
void Player::SetShapeShift(uint8 ss)
{
	...
	for( itr = mSpells.begin(); itr != mSpells.end(); ++itr )
	{
		...
		if( sp->apply_on_shapeshift_change || sp->Attributes & 64 )		// passive/talent
		{
			...
			{
				spe = new Spell( this, sp, true, nullptr );
				spe->prepare( &t );
			}
		}
	}
...
//////no druid ss	
		...
		if( it )
		{
			if( TO_PLAYER( this )->m_wratings.size() )
			{
...
void Player::UpdateComboPoints()
{
	...
	if(m_comboTarget != 0)
	{	
		Unit* target = (m_mapMgr != nullptr) ? m_mapMgr->GetUnit(m_comboTarget) : NULLUNIT;
	...
	SetUInt64Value(PLAYER__FIELD_COMBO_TARGET,m_comboTarget);
...
void Player::RemoveFromBattlegroundQueue(uint32 queueSlot, bool forced)
{
	...
	{
		m_pendingBattleground[queueSlot]->RemovePendingPlayer(this);
		m_pendingBattleground[queueSlot] = nullptr;
	}
	m_bgIsQueued[queueSlot] = false;
	BattlegroundManager.SendBattlegroundQueueStatus(this, queueSlot);
	sEventMgr.RemoveEvents(this, EVENT_BATTLEGROUND_QUEUE_UPDATE_SLOT_1 + queueSlot);
...
void Player::_UpdateSkillFields()
{
	...
	/* nullptr out the rest of the fields */
...
uint8 Player::_GetProfesionsAmount()
{
	uint8 i = 0;
...
void PlayerSkill::Reset(uint32 Id)
{
	...
	CurrentValue = 0;
	BonusValue = 0;
	Skill = (Id == 0) ? nullptr : dbcSkillLine.LookupEntry(Id);
}
...
void Player::_AddLanguages(bool All)
{
	...
	static uint32 skills[] = { SKILL_LANG_COMMON, SKILL_LANG_ORCISH, SKILL_LANG_DWARVEN, SKILL_LANG_DARNASSIAN, SKILL_LANG_TAURAHE, SKILL_LANG_THALASSIAN,
		SKILL_LANG_TROLL, SKILL_LANG_GUTTERSPEAK, 0 };
...
void Player::_LoadInventoryLight(uint32 guid)
{
	QueryResult* result = CharacterDatabase.Query("SELECT entry, containerslot, slot FROM playeritems WHERE ownerguid='%i' AND slot<'19'", guid);
	if(!result)
		return;
	do
        {
		...
		int8 container_slot = fields[1].GetUInt32();
		uint8 slot = fields[2].GetUInt32();
		GetItemInterface()->SafeAddItem(entry, container_slot, slot);
		}	
        while( result->NextRow() );
}
...
void Player::RemoveSpellTargets(uint32 Type)
{
	...
	{
		Unit * pUnit = m_mapMgr ? m_mapMgr->GetUnit(m_spellIndexTypeTargets[Type]) : nullptr;
		if(pUnit)
...
{
	CharacterDatabase.Execute("UPDATE characters SET pvprank = 0, selected_pvp_title = 0, available_pvp_title = 0 WHERE guid = %u",
		uint32(m_uint32Values[OBJECT_FIELD_GUID]);
}
...
void Player::EventSummonPet( Pet *new_pet )
{
	...
	for(iter= mSpells.begin();iter != mSpells.end();)
	{
		...
		if( spellInfo->c_is_flags & SPELL_FLAG_IS_CASTED_ON_PET_SUMMON_PET_OWNER )
		{
			...
			Spell *spell = new Spell(this, spellInfo ,true, nullptr);	//we cast it as a proc spell, maybe we should not !
			spell->prepare(&targets);
		}
...
void Player::AddShapeShiftSpell(uint32 id)
{
	...
	{
		Spell * spe = new Spell( this, sp, true, nullptr );
...
void Player::Cooldown_AddStart(SpellEntry * pSpell)
{
	...
	if( atime <= 0 )
		return;
	if( atime >= 1.0 )
		atime = 1.0; // global cooldown is decreased by spell haste, but it's not INCREASED by spell slow.
...
void Player::Social_AddFriend(const char * name, const char * note)
{
	...
	if( note != nullptr )
		...
		m_friends.insert( make_pair(info->guid, (char*)nullptr) );
...
void Player::Social_SendFriendList(uint32 flag)
{
	...
	//data << flag;
	data << uint8( m_friends.size());// + m_ignores.size() );
...
uint32 Player::CalculateTotalKills(Unit *Victim,uint32 fromDate,uint32 toDate) const
{
    ...
    {
        case TYPEID_PLAYER:
			ID = TO_PLAYER(Victim)->GetLowGUID();
        break;
...
void Player::UpdateHonor()
{
	...
	uint8 drunk = ((GetUInt32Value(PLAYER_BYTES_3) >> 24) & 0xFF); //needed for PLAYER_BYTES_3 update
    ...
    for (HonorCPMap::iterator itr = m_honorCP.begin(); itr != m_honorCP.end(); ++itr)
    {
        ...
        if (itr->type == HONORABLE)
        {
			...
			sLog.outError("thisWeekBegin = %i",thisWeekBegin);
			sLog.outError("thisWeekEnd = %i",thisWeekEnd);
    ...
    //STANDING
    SetHonorLastWeekStandingPos(objmgr.GetHonorStandingPositionByGUID(GetLowGUID(),GetTeam()) );

    //RANK POINTS
	HonorStanding *standing = objmgr.GetHonorStandingByGUID(GetLowGUID(),GetTeam());
	...
	//SetInt32Value(PLAYER_FIELD_BYTES2, RP); <-- mangos one
	SetUInt32Value(PLAYER_FIELD_BYTES2, RP); //NEEDS testing
    //RANK (Patent)
	if(GetHonorRankInfo().rank > 0)
		SetUInt32Value(PLAYER_BYTES_3, ( (GetHonorRankInfo().rank + 4) << 24) + (drunk & 0xFFFE) + getGender());
	else SetUInt32Value(PLAYER_BYTES_3, ( GetHonorRankInfo().rank << 24) + (drunk & 0xFFFE) + getGender());
    //TODAY
...
void Player::ResetHonor()
{
    ...
    CharacterDatabase.Execute("DELETE FROM character_honor_cp WHERE guid = '%u'",GetLowGUID());
    m_honorCP.clear();
...
bool Player::CalculateHonor(Unit *uVictim,uint32 groupsize)
{
    ...
    if (uVictim->HasAura(2479))             // Honorless Target
        return false;

    if (uVictim->GetTypeId() == TYPEID_UNIT)
    {
        Creature *cVictim = (Creature *)uVictim;
        //TODO: research on that is civillian and racial leader thing
		if (cVictim->GetCreatureInfo() && cVictim->GetCreatureInfo()->Civilian >= 1)
        {
            AddHonorCP(MaNGOS::Honor::DishonorableKillPoints(getLevel()),DISHONORABLE,cVictim->GetEntry(),TYPEID_UNIT);
            return true;
        }

		if (cVictim->GetCreatureInfo() && cVictim->GetCreatureInfo()->Leader >= 1)
        {
    ...
    if( uVictim->GetTypeId() == TYPEID_PLAYER )
    {
        ...
        if( getLevel() < (pVictim->getLevel()+10) )
        {
			AddHonorCP( MaNGOS::Honor::HonorableKillPoints( this, pVictim, groupsize),HONORABLE,pVictim->GetLowGUID(),TYPEID_PLAYER);
            return true;
        }
    }
...
bool Player::AddHonorCP(float honor,uint8 type,uint32 victim,uint8 victimType)
{
    ...
    }else
	{
		...
		uint32 int_honor = honor;
		data << int_honor << uint64(victim) << uint32(5);
		GetSession()->SendPacket(&data);
	}

    CP.state  =  HK_NEW;
    CP.isKill =  (victimType == TYPEID_UNIT || victimType == TYPEID_PLAYER );
...
void Player::_LoadHonorCP(QueryResult *result)
{
    if(result)
    {
        ...
        do
        {
            ...
            CP.isKill           = (CP.victimType == TYPEID_UNIT || CP.victimType == TYPEID_PLAYER );
    ...
    for (HonorCPMap::iterator itr = m_honorCP.begin(); itr != m_honorCP.end() ; ++itr)
    {
        ...
        switch(itr->state)
        {
            case HK_OLD:
                ...
                break;
            case HK_NEW:
                CharacterDatabase.Execute("INSERT INTO character_honor_cp (guid,victim_type,victim,honor,date,type) "
                    " VALUES (%u,%u,%u,%f,%u,%u)", GetLowGUID(), itr->victimType, itr->victimID, itr->honorPoints , itr->date, itr->type);
