...
#include "ClassSpells.h"
		...
		static uint32 paladinspellarray[RACE_TROLL+1][2] = {
			{ 0 },		// RACE 0
			{ 13819, 0 },		// HUMAN
			{ 0 },		// ORC
			{ 13819, 0 },		// DWARF
		...
		static uint32 priestspellarray[RACE_TROLL+1][16] = {
			{ 0 },		// RACE 0
		...
		static uint32 shamanspellarray[RACE_TROLL+1][2] = {
			{ 0 },		// RACE 0
			{ 0 },		// HUMAN
			{ 2825, 0 },		// ORC
			{ 0 },		// DWARF
		...
		static uint32 magespellarray[RACE_TROLL+1][11] = {
			{ 0 },		// RACE 0
...
bool ChatHandler::HandleGMTicketDelByIdCommand(const char* args, WorldSession *m_session)
{
	...
	if(guid != -1)
	{
		...
		chn->Say(m_session->GetPlayer(), str.str().c_str(), nullptr, true);
...
{
	char msg[512];
	uint32 guid = Chrono::Util::GUID_LOPART(m_session->GetPlayer()->GetSelection());
...
/*	if(!args || strlen(args) < 2)
	...
	pCreature->Load(sp, (uint32)nullptr, nullptr);
	...
	pPet->CreateAsSummon(Entry, pCreatureInfo, pCreature, plr, nullptr, 0x2, 0);
...
bool ChatHandler::HandlePetSpawnAIBot(const char* args, WorldSession *m_session)
{
	...
	if( race == RACE_HUMAN || race == RACE_DWARF || race == RACE_NIGHTELF || race == RACE_GNOME )
	{
		Entry = 1826;
...
bool ChatHandler::HandleFormationLink2Command(const char* args, WorldSession * m_session)
{
	...
	if( slave->GetAIInterface()->m_formationLinkTarget->m_spawn == nullptr || slave->m_spawn == nullptr )
		return false;
...
bool ChatHandler::HandleFormationClearCommand(const char* args, WorldSession * m_session)
{
	...
	if(!c || c->m_spawn == nullptr) return true;
...
bool ChatHandler::HandleCreatureSpawnCommand(const char *args, WorldSession *m_session)
{
	...
	{
		p->Load(proto, m_session->GetPlayer()->GetPositionX(), m_session->GetPlayer()->GetPositionY(), m_session->GetPlayer()->GetPositionZ());
	}
...
bool ChatHandler::HandleGORotate(const char * args, WorldSession * m_session)
{
	...
	go->ModFloatValue(GAMEOBJECT_ROTATION_1, rad);
	go->ModFloatValue(GAMEOBJECT_ROTATION_2, rad);
	go->ModFloatValue(GAMEOBJECT_ROTATION_3, rad);
	go->SaveToDB();
