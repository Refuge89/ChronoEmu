...
Unit::Unit()
{
	...
	m_meleespell = 0;
	m_addDmgOnce = 0;
	m_TotemSlots[0] = nullptr;
	m_TotemSlots[1] = nullptr;
	m_TotemSlots[2] = nullptr;
	m_TotemSlots[3] = nullptr;
	m_ObjectSlots[0] = 0;
	m_ObjectSlots[1] = 0;
	m_ObjectSlots[2] = 0;
	m_ObjectSlots[3] = 0;
...
//	fearSpell = 0;
	...
	//new extra target system backported from 3.3.2 core
...
/* Please dont do temp fixes. Better report to me. Thx. Shady */
		...
		if(m_aiInterface != nullptr && m_useAI)
...
void Unit::HandleProc( uint32 flag, Unit* victim, SpellEntry* CastingSpell, uint32 dmg, uint32 abs , bool mainhand )
{
	...
	for( itr = m_procSpells.begin(); itr != m_procSpells.end(); )  // Proc Trigger Spells for Victim
	{
		...
		if( itr2->procFlags & flag )
		{
			...
			if( itr2->procFlags & CPROC_CAST_SPECIFIC )
			{
			...
			if( IsPlayer() )
			{
				if( ospinfo->EquippedItemClass > -1 && ospinfo->EquippedItemSubClass > -1)
				{
					Item * itm;
			...
			//todo: redo enchanment procs! (might not be neccesary in 1.12.1)
			/*if( this->IsPlayer() && spe != nullptr )
			{
				float ppm = 1.0f;
				...
				Item * mh = TO_PLAYER( this )->GetItemInterface()->GetInventoryItem( EQUIPMENT_SLOT_MAINHAND );
				Item * of = TO_PLAYER( this )->GetItemInterface()->GetInventoryItem( EQUIPMENT_SLOT_OFFHAND );
				...
				{
					if( TO_PLAYER( this )->GetShapeShift() == FORM_CAT )
					{
						proc_Chance = FL2UINT( ppm / 0.6f );
					}
					else if( TO_PLAYER( this )->GetShapeShift() == FORM_BEAR || TO_PLAYER( this )->GetShapeShift() == FORM_DIREBEAR )
					{
			...
			if( spellId && Rand( proc_Chance ) )
			{
				...
				if( itr2->procFlags & CPROC_ON_SELF )
				...
				if( itr2->procFlags & CPROC_REMOVEONUSE )
					RemoveAura( origId );
				int dmg_overwrite = 0;
						...
						case 25505:
							{
								...
								uint32 dmg = TO_PLAYER( this )->GetMainMeleeDamage(AP_owerride);
						...
						case 14181:
							{
								...
								if(!Rand(20)) //why do we need to do this?
									continue;
							}break;
						//rogue - Initiative
						case 13977:
...
void Unit::RegenerateHealth()
{
	...
	if(this->IsPlayer())
	{
		...
		{
			TO_PLAYER( this )->RegenerateHealth(false);
		}
		else
			TO_PLAYER( this )->RegenerateHealth(true);
	}
	else
	{
		// Only regen health out of combat
		if(!CombatStatus.IsInCombat())
			static_cast<Creature*>(this)->RegenerateHealth(false);
	}
}
...
void Unit::RegeneratePower(bool isinterrupted)
{
	...
	if(this->IsPlayer())
	{
		...
		case POWER_TYPE_MANA:
			//if(!CombatStatus.IsInCombat())	//--You sure we need to remove this BT?
		...
		case POWER_TYPE_RAGE:
			{
				if(!CombatStatus.IsInCombat())
				{
					...
					if (HasAura(29131)) 
					{	
						TO_PLAYER(this)->LoseRage(0);
					}
					else if (TO_PLAYER(this)->HasAura(12296))
					{
				...
				{
					if (TO_PLAYER(this)->HasAura(12296))
					{
	...
	else
	{
		...
		{
		case POWER_TYPE_MANA:
			static_cast<Creature*>(this)->RegenerateMana(false);
			break;
...
//		double Reduction = double(pVictim->GetResistance(0)) / double(pVictim->GetResistance(0)+400+(85*getLevel()));
		...
		else if(getLevel() == 60) Reduction = double(pVictim->GetResistance(0) - ArmorReduce) / double(pVictim->GetResistance(0)-22167.5+(467.5*getLevel()));
		//
...
uint32 Unit::GetSpellDidHitResult( Unit* pVictim, uint32 weapon_damage_type, SpellEntry* ability )
{
	...
	{	  
		...
		case MELEE:   // melee main hand weapon
			it = disarmed ? nullptr : pr->GetItemInterface()->GetInventoryItem( EQUIPMENT_SLOT_MAINHAND );
		...
		case OFFHAND: // melee offhand weapon (dualwield)
			it = disarmed ? nullptr : pr->GetItemInterface()->GetInventoryItem( EQUIPMENT_SLOT_OFFHAND );
			hitmodifier += pr->CalcRating( PLAYER_RATING_MODIFIER_MELEE_HIT );
			self_skill = float2int32( pr->CalcRating( PLAYER_RATING_MODIFIER_MELEE_OFF_HAND_SKILL ) );
			break;
		case RANGED:  // ranged weapon
			it = disarmed ? nullptr : pr->GetItemInterface()->GetInventoryItem( EQUIPMENT_SLOT_RANGED );
	...
	else if(this->IsPlayer())
	{
		...
		if( it != nullptr && it->GetProto()->InventoryType == INVTYPE_WEAPON && !ability )//dualwield to-hit penalty
		{
			hitmodifier -= 19.0f;
		}
		else
		{
			it = TO_PLAYER( this )->GetItemInterface()->GetInventoryItem( EQUIPMENT_SLOT_MAINHAND );
			if( it != nullptr && it->GetProto()->InventoryType == INVTYPE_2HWEAPON )//2 handed weapon to-hit penalty
				hitmodifier -= 4.0f;
		}
	}
...
//--------------------------------mob defensive chances-------------------------------------
	else
	{
		...
		{ 
			...
			if( c != nullptr && c->GetCreatureName()&&c->GetCreatureName()->Rank == ELITE_WORLDBOSS )
			{
...
//--------------------------------by ratings------------------------------------------------
	...
	if (this->IsPlayer())
	{
		...
		dodge -=TO_PLAYER(this)->CalcRating( PLAYER_RATING_MODIFIER_EXPERTISE );
		if(dodge<0) dodge=0.0f;
		parry -=TO_PLAYER(this)->CalcRating( PLAYER_RATING_MODIFIER_EXPERTISE );
...
//--------------------------------by damage type and by weapon type-------------------------
	...
	if(  this->IsPlayer() && ability && TO_PLAYER( this )->m_finishingmovesdodge && ability->c_is_flags & SPELL_FLAG_IS_FINISHING_MOVE)
			dodge = 0.0f;
	...
	if( ability != nullptr && ability->Attributes & ATTRIBUTES_CANT_BE_DPB )
	{
		dodge = 0.0f;
		parry = 0.0f;
		block = 0.0f;
...
//--------------------------------miss------------------------------------------------------
	case 0:
		...
		// dirty ai agro fix
		if(pVictim->GetTypeId() == TYPEID_UNIT && pVictim->GetAIInterface()->GetNextTarget() == nullptr)
	...
	case 1:
		...
		if( pVictim->IsPlayer() )
		{
			...
			if( this->IsPlayer() && TO_PLAYER( this )->getClass() == WARRIOR )
			{
				...
				if( !sEventMgr.HasEvent( TO_PLAYER( this ), EVENT_COMBO_POINT_CLEAR_FOR_TARGET ) )
					sEventMgr.AddEvent( TO_PLAYER( this ), &Player::NullComboPoints, (uint32)EVENT_COMBO_POINT_CLEAR_FOR_TARGET, (uint32)5000, (uint32)1, (uint32)0 );
				else
					sEventMgr.ModifyEventTimeLeft( TO_PLAYER( this ), EVENT_COMBO_POINT_CLEAR_FOR_TARGET, 5000 ,0 );
			}
...
//--------------------------------not miss,dodge or parry-----------------------------------
	default:
		...
		if( pVictim->IsImmuneForCustomEffect(0) )
			vstate = IMMUNE;		
		else
		{
//--------------------------------state proc initialization---------------------------------
			vproc |= PROC_ON_TAKE_DAMAGE;			
			if( weapon_damage_type != RANGED )
			{
				aproc |= PROC_ON_MELEE_HIT;
				vproc |= PROC_TARGET_MELEE_HIT;
			}
			else
			{
				aproc |= PROC_ON_RANGED_HIT;
				vproc |= PROC_TARGET_RANGED_HIT;
				if(ability && ability->Id==3018 && IsPlayer() && getClass()==HUNTER)
					aproc |= CPROC_AUTO_SHOT;
			}
//--------------------------------base damage calculation-----------------------------------
			...
			//a bit dirty fix
			/*if( ability != nullptr && ability->NameHash == SPELL_HASH_SHRED )
			{
			...
			case 4:
				{
					...
					if( shield != nullptr )
					{
						...
						pVictim->HandleProcDmgShield(PROC_ON_BLOCK, TO_UNIT(this));
					}
				}
				break;
//--------------------------------critical hit----------------------------------------------
			case 5:
				{
					...
					if(IsPlayer())
					{
						if( weapon_damage_type != RANGED )
						{
							float critextra=float(TO_PLAYER( this )->m_modphyscritdmgPCT);
							dmg.full_damage += int32((float(dmg.full_damage)*critextra/100.0f));
						}
						if(!pVictim->IsPlayer())
							dmg.full_damage += float2int32(dmg.full_damage*TO_PLAYER( this )->IncreaseCricticalByTypePCT[((Creature*)pVictim)->GetCreatureName() ? ((Creature*)pVictim)->GetCreatureName()->Type : 0]);
					...
					vproc |= PROC_TARGET_MELEE_CRIT;
					aproc |= PROC_ON_MELEE_CRIT;
					if( weapon_damage_type == RANGED )
					{
						vproc |= PROC_TARGET_RANGED_CRIT;
						aproc |= PROC_ON_RANGED_CRIT;
					}
...
//--------------------------spells triggering-----------------------------------------------
	if(realdamage > 0 && ability == 0)
	{
		if( IsPlayer() && TO_PLAYER( this )->m_onStrikeSpells.size() )
		{
				...
				itr != TO_PLAYER( this )->m_onStrikeSpells.end(); ++itr )
			{
				if( itr->second.first )
				{
					...
					// Cast.
					cspell = new Spell(this, itr->first, true, nullptr);
		...
		{
			if( pVictim->m_auras[x] != nullptr && pVictim->m_auras[x]->GetUnitCaster() != nullptr && pVictim->m_auras[x]->GetUnitCaster()->GetGUID() == GetGUID() && pVictim->m_auras[x]->GetSpellProto()->buffIndexType == SPELL_TYPE_INDEX_JUDGEMENT )
			{
				Aura * aur = pVictim->m_auras[x];
				SpellEntry * spinfo = aur->GetSpellProto();
				aur->Remove();
				Spell * sp = new Spell( this , spinfo , true , nullptr );
				...
				*/
				...
				// I would say break; here, but apparently in ChronoEmu, one paladin can have multiple judgements on the target. No idea if this is blizzlike or not.
			}
		}
...
//--------------------------extra strikes processing----------------------------------------
	if(!m_extraAttackCounter)
	{
		...
		{
			extra_attacks--;
			Strike( pVictim, weapon_damage_type, nullptr, 0, 0, 0, false, false);
		}
	...
	if(m_extrastriketargets)
	{
		...
		for(set<Object*>::iterator itr = m_objectsInRange.begin(); itr != m_objectsInRange.end() && m_extra; ++itr)
		{
			...
			{
				Strike( TO_UNIT( *itr ), weapon_damage_type, ability, add_damage, pct_dmg_mod, exclusive_damage, false ,false );
				--m_extra;
			}
		}
	...
	if(m_extrastriketargetc > 0 && m_extrastriketarget == 0)
	{
		...
		for(std::list<ExtraStrike*>::iterator itx = m_extraStrikeTargets.begin();itx != m_extraStrikeTargets.end(); itx++)
		{
			...
			for(set<Object*>::iterator itr = m_objectsInRange.begin(); itr != m_objectsInRange.end(); ++itr)
			{
				...
				if(CalcDistance(*itr) < 5.0f && isAttackable(TO_UNIT(this), (*itr)) && this->isInFront((*itr)) && !TO_UNIT(*itr)->IsPacified())
				{
					// Sweeping Strikes hits cannot be dodged, missed or parried (from wowhead)
					bool skip_hit_check = ex->spell_info->Id == 12292 ? true : false;
...
{
	if(aur->GetSpellProto()->School && IsImmuneFor(aur->GetSpellProto()->School))
    {
        delete aur;
        return;
    }
	...
	if( aur->m_auraSlot != 0xffffffff )
		m_auras[aur->m_auraSlot] = nullptr;   
	...
	// We add 500ms here to allow for the last tick in DoT spells. This is a dirty hack, but at least it doesn't crash like my other method.
 	...
 	if(aur->GetDuration() > 0)
 	{
		...
		sEventMgr.AddEvent(aur, &Aura::Remove, true, EVENT_AURA_REMOVE, aur->GetDuration() + 500, 1,
...
void Unit::RemoveAllNegativeAuras()
{
	for(uint32 x=MAX_POSITIVE_AURAS;x<MAX_AURAS;x++)
	{
		...
		{
			if(m_auras[x]->GetSpellProto()->Flags4 & FLAGS4_DEATH_PERSISTANT)
				continue;
			else
			{
			RemoveAuraBySlot(x);
...
{   
	...
	for(uint32 x=0;x<MAX_AURAS+MAX_PASSIVE_AURAS;x++)
	{
		if(m_auras[x] && (ignore == nullptr || ignore != m_auras[x]))
		{
			if(m_auras[x]->GetSpellId()==spellId)
			{
...
//------------------------------by victim type----------------------------------------------
	if(((Creature*)pVictim)->GetCreatureName() && caster->IsPlayer()&& !pVictim->IsPlayer())
		plus_damage += TO_PLAYER(caster)->IncreaseDamageByType[((Creature*)pVictim)->GetCreatureName()->Type];
//==========================================================================================
...
//------------------------------by cast duration--------------------------------------------
	...
	else
	{
		...
		else
		{
			...
			if( spellInfo->NameHash == SPELL_HASH_MOONFIRE || spellInfo->NameHash == SPELL_HASH_IMMOLATE || spellInfo->NameHash == SPELL_HASH_PYROBLAST )
	...
	if(spellInfo->baseLevel > 0 && spellInfo->maxLevel > 0)
	{
		...
		float downrank2 = ( float(spellInfo->maxLevel + 5.0f) / float(TO_PLAYER(caster)->getLevel()) );
...
void Unit::SendChatMessageToPlayer(uint8 type, uint32 lang, const char *msg, Player *plr)
{
	...
	CreatureInfo *ci = (m_objectTypeId == TYPEID_UNIT) ? ((Creature*)this)->creature_info : nullptr;

	if(ci == nullptr || plr == nullptr)
		return;
...
{
	if(!IsPlayer())	// Just in case
		return;	
		
	int32 perc;
	uint32 heal;	
	// Fuck I hate doing it like this
	// It checks the aura, since the auras won't modify the health 
	if (HasAura(27839))
		perc = 25;
	else if (HasAura(27840))
		perc = 30;
	else
		perc = 20;
		
	// Once again I hate to do this, 
	// but I want to make sure calculations are right
	if (HasAura(27839))				//Use 97, otherwise you only heal
		heal = (dmg*perc) / 97;		//for about 20%. 97 gives you the full 25%
	else if (HasAura(27839))		//Use 120 Otherwise you heal 
		heal = (dmg*perc) / 120; 	//for 200ish HP out of a 700 - 800 Mind blast
	else							
		heal = (dmg*perc) / 100;	// This is the correct 20%, it works with no changes
		
		
	SM_FIValue(SM_FSPELL_VALUE,&perc,4);
	this->Heal(this,15286,heal);

	SubGroup* pGroup = TO_PLAYER( this )->GetGroup() ?
		TO_PLAYER( this )->GetGroup()->GetSubGroup( TO_PLAYER( this )->GetSubGroup() ) : nullptr;

	if( pGroup != nullptr )
	{
		GroupMembersSet::iterator itr;
		TO_PLAYER( this )->GetGroup()->Lock();
		...
		TO_PLAYER(this)->GetGroup()->Unlock();
	}
...
void Unit::SetStandState(uint8 standstate)
{
	...
	if( m_objectTypeId == TYPEID_PLAYER )
		TO_PLAYER( this )->GetSession()->OutPacket( SMSG_STANDSTATE_CHANGE_ACK, 1, &standstate );
}
...
void Unit::RemoveAurasByInterruptFlag(uint32 flag)
{
	...
	for(uint32 x=0;x<256;x++)
	{
		...
		if((a->m_spellProto->AuraInterruptFlags & flag) && !(a->m_spellProto->procFlags & CPROC_REMOVEONUSE))
		{
			a->Remove();
			m_auras[x] = nullptr;
		}
...
void Unit::DropAurasOnDeath()
{
	for(uint32 x=0;x<MAX_AURAS;x++)
    {
        if(m_auras[x])
        {
            if(m_auras[x] && m_auras[x]->GetSpellProto()->Flags4 & FLAGS4_DEATH_PERSISTANT)
                continue;
            else
	        {
		        m_auras[x]->Remove();
...
void Unit::EventSummonPetExpire()
{
	if(summonPet)
	{
		if(summonPet->GetEntry() == 7915)//Goblin Bomb
		{
			...
			Spell*sp=new Spell(summonPet,spInfo,true,nullptr);
			SpellCastTargets tgt;
...
{
	Spell * pSpell = new Spell(Target, Sp, true, nullptr);
...
void CombatStatusHandler::RewardHonorForAttackers()
{
	map<uint64,uint32>::iterator i = m_Unit->CombatStatus.DamageMap.find(m_Unit->GetGUID());
	Unit * pt;

	uint8 players = 0;

	sLog.outError(" m_Unit->CombatStatus.DamageMap = %i",m_Unit->CombatStatus.DamageMap.size());

	for(i = m_Unit->CombatStatus.DamageMap.begin(); i != m_Unit->CombatStatus.DamageMap.end(); ++i)
	{
		pt = m_Unit->GetMapMgr()->GetUnit((*i).first);
		if(pt && pt->IsPlayer() && pt->isInRange(m_Unit, 100.0f))
		{
			sLog.outError("we got a player");
			players++;
		}
	}
	...
	for(i = m_Unit->CombatStatus.DamageMap.begin(); i != m_Unit->CombatStatus.DamageMap.end(); ++i)
	{
		...
		if(pt && pt->IsPlayer() && pt->isInRange(m_Unit, 100.0f))
		{
			TO_PLAYER(pt)->CalculateHonor(m_Unit, players);
		}
	}
}
...
{
	Unit * victim = m_mapMgr ? m_mapMgr->GetUnit(guid) : nullptr;
	if(victim)
...
{
	if(m_auras[Slot]!=nullptr)
	{
		m_auras[Slot]->Remove();
		m_auras[Slot] = nullptr;
	}
}
...
void Unit::AddImmuneEffect(uint8 immune)
{
	map<uint32, uint32>::iterator itr = MechanicsDispels.find(immune);
	if(itr != MechanicsDispels.end())
		(*itr).second++;
	else
		MechanicsDispels.insert(make_pair(immune, 1));
}

bool Unit::IsImmuneFor(uint8 immune)
{
	if(IsCreature() && TO_CREATURE(this)->GetProto())
	{
		if(TO_CREATURE(this)->GetProto()->immune_mask !=0 && immune != 0)
		{
			if(TO_CREATURE(this)->GetProto()->immune_mask & float2int32(pow(2.0f, (immune-1))))
				return true;
		}else if(immune == 0) return false;
	}
...
void Unit::RemoveImmuneEffect(uint8 immune)
{
	map<uint32, uint32>::iterator itr = MechanicsDispels.find(immune);
	if(itr != MechanicsDispels.end())
	{
		(*itr).second--;
		if((*itr).second == 0)
			MechanicsDispels.erase(itr);
	}
}

/* Custom Immune */

void Unit::AddCustomImmuneEffect(uint8 effect)
{
	map<uint32, uint32>::iterator itr = CustomImmunityList.find(effect);
	if(itr != CustomImmunityList.end())
		(*itr).second++;
	else
		CustomImmunityList.insert(make_pair(effect, 1));
}
bool Unit::IsImmuneForCustomEffect(uint8 effect)
{
	if(IsCreature() && TO_CREATURE(this)->GetProto())
	{
		if(TO_CREATURE(this)->GetProto()->custom_immune !=0)
		{
			if(TO_CREATURE(this)->GetProto()->custom_immune & float2int32(pow(2.0f, effect)) )
				return true;
		}else return false;
	}
...
void Unit::RemoveCustomEffect(uint8 effect)
{
	map<uint32, uint32>::iterator itr = CustomImmunityList.find(effect);
	if(itr != CustomImmunityList.end())
	{
