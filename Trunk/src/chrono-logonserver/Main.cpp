...
#define BANNER "Chrono Emulator r%u/%s-%s (%s) :: Logon Server\n"
...
#else
	char * config_file = (char*)CONFDIR "/chrono-logonserver.conf";
#endif
...
void LogonServer::Run(int argc, char ** argv)
{
	...
	/*if(Config.MainConfig.SetSource(config_file))
	{
		Log.Success("Config", ">> chrono-logonserver.conf", config_file);
	}
	else
	{
		Log.Error("Config", ">> chrono-logonserver.conf", config_file);
		return;
	}*/
	if(!Rehash())
		return;
